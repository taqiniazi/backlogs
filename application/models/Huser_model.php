 ...
 */
class Huser_model extends CI_Model
{
	/**
	...
	public function get_update_details($user_type='', $user_id='', $user_group_id='',$group_id=''){
		$up1 = array('group_id'=>$group_id);

		$this->db->where('id', $user_group_id);
		$this->db->update('users_groups', $up1);

		$up2 = array('user_type'=>$user_type);
		$this->db->where('id', $user_id);
		$update = $this->db->update('users', $up2);
		return $update;
	}

	function get_gourp_details($group_id=''){
		$this->db->where('id', $group_id);
		$this->db->where('type_cate', 'category');
	...
	/**
	 * get Hospital division user count
	 * @return    bool
	 * @author    Ajendra
	 */
	function get_divison_user_count($division_id=''){
		...
		$res = $this->db->select("users.id,users.division_id,users.department_id, users_groups.institute_id,users_groups.user_id as guserid, users_groups.id as ugid")
		->from('users')
	...
	function get_hospital_users($userType='', $countType='') {
		...
		$hospital_id = $hospital_row->id;
		if($userType=='all'){
			$whereArray = array('institute_id'=>$hospital_id);

		}else if($userType){
			$whereArray = array('institute_id'=>$hospital_id,'group_id'=>$userType);
		}else {
			$whereArray = array('institute_id'=>$hospital_id);
		}
		...
		$res = $this->db
		->select("
			`users`.`id` as id,`users_groups`.`id` as user_group_id, 
    ...
    public function register($identity, $password, $email, $additional_data = [], $groups = [])
    {
    	...
    	$data = [
    		...
    		'created_on' => $this->db->escape("$time"),
    		'active' => $this->db->escape('1')
    	];
    	...
    	$query = "INSERT INTO " . $this->tables['users'] . " (first_name, last_name, company, phone, user_type, memorable, email, username, password, ip_address, created_on, active, profile_picture_path, is_hospital_admin)
    	VALUES (AES_ENCRYPT(" . $user_data['first_name'] . ", '" . DATA_KEY . "' ), AES_ENCRYPT(" . $user_data['last_name'] . ", '" . DATA_KEY . "' ), AES_ENCRYPT(" . $user_data['company'] . ", '" . DATA_KEY . "' )
    	,AES_ENCRYPT(" . $user_data['phone'] . ", '" . DATA_KEY . "' ), " . $userType . "," . $user_data['memorable'] . ", AES_ENCRYPT(" . $user_data['email'] . ", '" . DATA_KEY . "' )
    	, AES_ENCRYPT(" . $user_data['username'] . ", '" . DATA_KEY . "' ), '" . $user_data['password'] . "', '" . $user_data['ip_address'] . "', " . $user_data['created_on'] . ", " . $user_data['active'] . ", ".$additional_data['profile_picture_path'] .", ".$is_hospital_admin.")";
    	$this->db->query($query);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		->where($this->tables['users_groups'] . '.' . $this->join['groups']. ' != ', 'null')
    ...
    {	
    	$result = $this->db->query("SELECT * FROM division_list")->result_array();
    	$hospital_row = $this->ion_auth->get_users_groups()->row();
    	$hospital_id = $hospital_row->id;
    	...
    	if(!empty($result)){
    		foreach($result as$key=> $res){
    			$div_id = $res['id'];
    			$result[$key]['department'] = $this->gethospitalDeptartment($hospital_id, $div_id);
    		}
    	}
    	return $result;
    }

    public function gethospitalDeptartment($h_id='', $div_id='')
    {
    	...
    	$flat_tree = $this->db->get_where('department_settings', array('hospital_id' => $h_id,'division_id'=>$div_id))->result_array();
    	...
    	foreach ($departments as $d_id => $dep) {
    		foreach ($dep['specialties'] as $s_id => $spec) {
    			$departments[$d_id]['specialties'][$s_id]['test_categories'] = $this->get_hospital_categories($h_id, $d_id, $s_id);
    		}
    	}
    ...
    public function get_hospital_categories($hospital_id, $department_id, $speciality_id) {
...
//        echo "<br>".$user_type; exit;
    	...
    	$sql .= "division_id=" . $this->db->escape($division_id) . ",";
        $sql .= "department_id=" . $this->db->escape($department_id) . ",";
        $sql .= "speciality_id=" . $this->db->escape($speciality_id) . ",";
        $sql .= "category_id=" . $this->db->escape($category_id) . ",";
    	$sql .= "user_type=" . $this->db->escape($user_type) . ",";
...
//        echo $this->db->last_query(); exit;
    	...
    	$this->updateUserGroupId($user_id, $main_group_id);
//        echo "HERES"; exit;
