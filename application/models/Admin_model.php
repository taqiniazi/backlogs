...
class Admin_model extends CI_Model
{
    ...
    public function get_doctors()
    {
        ...
        $sql = "SELECT AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone,AES_DECRYPT(company, '" . DATA_KEY . "') AS company,AES_DECRYPT(last_name, '" . DATA_KEY . "') AS last_name,AES_DECRYPT(first_name, '" . DATA_KEY . "') AS first_name, AES_DECRYPT(email, '" . DATA_KEY . "') AS email, id,AES_DECRYPT(username, '" . DATA_KEY . "') AS username, profile_picture_path FROM users WHERE users.user_type = 'D'";
        $query = $this->db->query($sql);
        return $query->result();
    }
...
//                                WHERE id=" . $this->db->escape($id));
    ...
    public function getLaboratoryHospitals($laboratoryId){
        $query = "select hi.group_id, groups.description,hi.hosp_city,hi.hosp_email,hi.hosp_phone, COUNT(patients.id) as patient_count
                  from hospital_information hi 
                  join groups on hi.group_id=groups.id
                  join hospital_group hg on hg.hospital_id=groups.id
                  left join patients on patients.hospital_id=hi.group_id
                  where hg.group_id=$laboratoryId 
                  group by hi.group_id";
        $result = $this->db->query($query)->result_array();
        ...
        foreach ($result as $k=>$row){
            ...
            $result[$k]['request_count'] = $this->db->query($subQuery)->row()->request_count;
        }
        return $result;
    ...
    public function getParentGroupsDetailsByUserId($user_id){
        $query = "SELECT groups.id, group_type, groups.name
                            FROM groups 
                            INNER JOIN users_groups ON groups.id = users_groups.institute_id 
                            WHERE users_groups.user_id=$user_id AND users_groups.institute_id IS NOT NULL";
