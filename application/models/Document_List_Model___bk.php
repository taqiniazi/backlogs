...
class Document_List_Model extends CI_Model
{
	...
	public function getmonthlyCategory()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        return $this->db->select('*')->from('monthly_category')
            ->get()->result();
    }
    ...
    public function getWorksheetCategory()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        return $this->db->select('*')->from('timesheet_category')
            ->get()->result();
    }

    public function getWorksheetCategory_new()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        return $this->db->select('*')->from('monthly_stainer_category')
            ->get()->result();
    }

    public function monthly_stainer_checklist($user_id, $fdate, $tdate)
    {         
        $to_date = $tdate;
        $from_date = $fdate;
        $query = $this->db->select('*')
                ->where('date_format(log_date, "%Y-%m-%d") >=', $from_date)
                ->where('date_format(log_date, "%Y-%m-%d") <=', $to_date)
                ->where("user_id",$user_id)
                ->order_by('id', 'ASC')
                ->get('monthly_stainer_checklist');        
        return $query->result_array();
    }
    public function get_weekly_request($user_id, $fdate, $tdate)
    {         
        $to_date = $tdate;
        $from_date = $fdate;
        $query = $this->db->select('*')
    ...
    public function get_weekly_request_new($user_id, $fdate, $tdate)
    {         
        $to_date = $tdate;
        $from_date = $fdate;
        $query = $this->db->select('*')
               ...
               // ->where("user_id",$user_id)
                ->order_by('id', 'ASC')
                ->get('timesheet_logs_new');   
				
				
        return $query->result_array();
    }
	
    public function getMonthlyStainer(){
        $user_id = $this->ion_auth->user()->row()->id;
        return $this->db->select('ms.*, CONCAT(AES_DECRYPT(u.first_name, "' . DATA_KEY . '"), " ", AES_DECRYPT(u.last_name, "' . DATA_KEY . '")) AS actionOwner, CONCAT(AES_DECRYPT(u1.first_name, "' . DATA_KEY . '"), " ",AES_DECRYPT(u1.last_name, "' . DATA_KEY . '")) AS addedBy')
                            ->from('monthly_stainer ms')                            
                            ->join('users u', 'u.id = ms.action_owner', 'left')
                            ->join('users u1', 'u1.id = ms.added_by', 'left')
                           // ->where('ms.added_by', $user_id)
                            ->get()->result_array();		
    }

    public function getMonthlyTask(){
        $user_id = $this->ion_auth->user()->row()->id;
        return $this->db->select('ms.*, CONCAT(AES_DECRYPT(u.first_name, "' . DATA_KEY . '"), " ", AES_DECRYPT(u.last_name, "' . DATA_KEY . '")) AS actionOwner, CONCAT(AES_DECRYPT(u1.first_name, "' . DATA_KEY . '"), " ",AES_DECRYPT(u1.last_name, "' . DATA_KEY . '")) AS addedBy')
                            ->from('monthly_task ms')                            
                            ->join('users u', 'u.id = ms.action_owner', 'left')
                            ->join('users u1', 'u1.id = ms.added_by', 'left')
                            //->where('ms.added_by', $user_id)
                            ->get()->result_array();		
