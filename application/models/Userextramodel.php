...
class Userextramodel extends CI_Model
{
    ...
    {
        $query = $this->db->query(" select aes_decrypt(users.first_name, '" . DATA_KEY . "') AS first_name,
        aes_decrypt(users.last_name, '" . DATA_KEY . "') AS last_name from users where id='$id'");
	...
	public function display_doctor_only_labbyID($id)
    {
        ...
        $query = $this->db->query(
            ...
            INNER JOIN `groups`
            WHERE users_request.doctor_id = $id
            AND `groups`.id = users_request.group_id
            AND `groups`.group_type = 'L'
...
//        $where .= " AND users.deleted=0";
  ...
  public function getAllusersForImprovementPage(){
        $query =
            "SELECT
        aes_decrypt(users.username, '" . DATA_KEY . "') AS enc_username,
        aes_decrypt(users.email, '" . DATA_KEY . "') AS enc_email,
        ...
        INNER JOIN users_groups ON users_groups.user_id = users.id
        INNER JOIN `groups` ON `users_groups`.`group_id` = `groups`.`id`
        where users_groups.group_id = 128
        ";
        $dataquery = $this->db->query($query);
