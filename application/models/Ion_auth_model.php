...
class Ion_auth_model extends CI_Model
{
	...
	public function identity_check($identity = '')
	{
		...
		$query = $this->db->query("SELECT COUNT(*) AS TOTROWS FROM " . $this->tables['users'] . "
						WHERE " . $this->identity_column . "= AES_ENCRYPT('" . $identity . "', '" . DATA_KEY . "')");
		$results = $query->row();
	...
	public function getregisterapiuser($email, $password)
	{
		if ($this->identity_check($email) > 0) {
			return 1;
			//return FALSE;
		}
		...
		$query = "INSERT INTO users ( email, password,user_type, created_on, active) 
		VALUES (AES_ENCRYPT(" . $this->db->escape($email) . ", '" . DATA_KEY . "' ),'" . $password . "','R' ," . $createdon . ", 1)";
		$this->db->query($query);
	 ...
	 * @author    Mathew
	 */
	public function loginapi($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->query("SELECT AES_DECRYPT(" . $this->identity_column . ", '" . DATA_KEY . "') AS email,id, password, active, last_login FROM users
		WHERE $this->identity_column= AES_ENCRYPT(" . $this->db->escape($identity) . ", '" . DATA_KEY . "')");
    ...
    public function check_existing_user($first_name, $last_name){

        $query = $this->db->query("SELECT id FROM users WHERE 
                              first_name = AES_ENCRYPT(" . $this->db->escape($first_name) . ", '" . DATA_KEY . "') AND
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		if ($this->identity_check($identity) > 0) {
		...
		// Users table.
		$data = [
			$this->identity_column => $this->db->escape($identity),
			'username' => $this->db->escape($identity),
			'password' => $password,
		...
		$default_avatar_path = "assets/img/user.jpg";
		$is_hospital_admin = 0;
		if (isset($additional_data['is_hospital_admin'])) {
			$is_hospital_admin = $additional_data['is_hospital_admin'];
		}
		$userType = $user_data['user_type'];
		
		$division   = (isset($additional_data['division_id'])) ? $additional_data['division_id'] : 0;
		$department = (isset($additional_data['department_id'])) ? $additional_data['department_id'] : 0;
		$spe        = (isset($additional_data['speciality_id'])) ? $additional_data['speciality_id'] : 0;
		$cate_id    = (isset($additional_data['category_id'])) ? $additional_data['category_id'] : 0;
        $clinic_id  = (isset($user_data['clinic_id'])) ? $user_data['clinic_id'] : 0;
//		$sub_role=($additional_data['sub_role']==""?'NULL':$additional_data['sub_role']);

        if($additional_data['group_id']==92){
            $sub_role=$additional_data['sub_role'];
        } else {
            $sub_role = 'NULL';
        }
		if(!$user_data['provider_no']){
			$user_data['provider_no'] = '';
		}
...
//		exit;
		if($user_data['provider_no']=='')
		{
			$user_data['provider_no']="111111111";
		}
		else
		{
			$user_data['provider_no']=$user_data['provider_no'];	
		}
		$tableName = $this->tables['users'];
        $user_data['user_auth_pass'] = ($user_data['user_auth_pass']!=""?$user_data['user_auth_pass']:"''");
		$query = "INSERT INTO $tableName (first_name, last_name, company, phone, user_type, memorable,user_auth_pass, email, username, password, ip_address, created_on, active, profile_picture_path, is_hospital_admin, division_id, department_id,speciality_id,category_id,sub_role, clinic_id, provider_no) 
                    VALUES (
                        AES_ENCRYPT(" . $user_data['first_name'] . ", '" . DATA_KEY . "'), 
                        AES_ENCRYPT(" . $user_data['last_name'] . ", '" . DATA_KEY . "'), 
                        AES_ENCRYPT(" . $user_data['company'] . ", '" . DATA_KEY . "'),
                        AES_ENCRYPT(" . $user_data['phone'] . ", '" . DATA_KEY . "'), 
                        $userType,
                        " . $user_data['memorable'] . ",
                        " . $user_data['user_auth_pass'] . ", 
                        AES_ENCRYPT(" . $user_data['email'] . ", '" . DATA_KEY . "'), 
                        AES_ENCRYPT(" . $user_data['username'] . ", '" . DATA_KEY . "'), 
                        '" . $user_data['password'] . "', 
                        '" . $user_data['ip_address'] . "', 
                        " . $user_data['created_on'].",
                        ".$user_data['active'] . ", 
                        ".$additional_data['profile_picture_path'] .", 
                        $is_hospital_admin,
                        $division,
                        $department,
                        $spe,
                        $cate_id,
                        $sub_role,
                        $clinic_id,".$user_data['provider_no'].")";
		@$this->db->query($query);
  ...
  //                      print_r($groups[0]); echo "<br>";
			if (isset($default_group->id) && $groups[0] == -1) {
...
//print_r($groups); exit; 
	...
	{
		//var_dump($this->session->userdata('identity'));exit;
		...
		if ($recheck !== 0) {
			...
			if ($last_login + $recheck < time()) {
				/*$query = $this->db->select('id')
								  ...
								  ->order_by('id', 'desc')
								  ->get($this->tables['users']);*/
				$query = $this->db->query("SELECT id FROM " . $this->tables['users'] . "
                                WHERE " . $this->identity_column . "= AES_ENCRYPT('" . $this->session->userdata('identity') . "', '" . DATA_KEY . "') ORDER BY id desc limit 1");
				if ($query->num_rows() === 1) {
	...
	public function users($groups = NULL)
	{
		...
		if (isset($this->_ion_select) && !empty($this->_ion_select)) {
			...
			$this->db->select([
				$this->tables['users'] . '.*',
				$this->tables[''] . 'aes_decrypt(users.username, "' . DATA_KEY . '") AS enc_username',
				$this->tables[''] . 'aes_decrypt(users.email, "' . DATA_KEY . '") AS enc_email',
				$this->tables[''] . 'aes_decrypt(users.first_name, "' . DATA_KEY . '") AS enc_first_name',
				$this->tables[''] . 'aes_decrypt(users.last_name, "' . DATA_KEY . '") AS enc_last_name',
				$this->tables[''] . 'aes_decrypt(users.company, "' . DATA_KEY . '") AS enc_company',
				$this->tables[''] . 'aes_decrypt(users.phone, "' . DATA_KEY . '") AS enc_phone',
	...
	/**
	 * @ Get the User Roles Data
	 * 
	 * 17 Nov 2016
	 */
	public function get_user_role_data($user_id)
	{
		$query = $this->db->query("SELECT * FROM user_roles WHERE user_roles.user_id = $user_id");
		return $query->result();
	}
	 ...
	 * @author Ben Edmunds
	 */
	public function getFiledByid($id, $type)
	{

		$query = $this->db->query("SELECT AES_DECRYPT('" . $type . "', '" . DATA_KEY . "') AS " . $type . "  FROM users
								WHERE id=" . $id);
		echo last_query();
		exit;
		//query->collumn_name
	...
	public function getUserDecryptedDetailsByid($id)
	{
		...
		$query = $this->db->query("SELECT AES_DECRYPT(email, '" . DATA_KEY . "') AS email, AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone,AES_DECRYPT(company, '" . DATA_KEY . "') AS company,AES_DECRYPT(last_name, '" . DATA_KEY . "') AS last_name,AES_DECRYPT(first_name, '" . DATA_KEY . "') AS first_name, AES_DECRYPT(email, '" . DATA_KEY . "') AS email, id,AES_DECRYPT(username, '" . DATA_KEY . "') AS username  FROM users
                                WHERE id=" . $id);
		//query->collumn_name
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description'.', group_type')
			->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)->where($this->tables['users_groups'] . '.' . $this->join['groups']. ' != ', 'null')
			->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.id')
			->get($this->tables['users_groups']);
	}
                
    public function get_users_main_groups($id = FALSE)
	{
		$this->trigger_events('get_users_group');
		...
		$id || $id = $this->session->userdata('user_id');
		 ...
		 return $this->db->select($this->tables['users_groups'] . '.' . 'institute_id' . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description'.', group_type')
			->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
			->where($this->tables['users_groups'] . '.' . 'institute_id'. ' != ', 'null')
			->join($this->tables['groups'], $this->tables['users_groups'] . '.' . 'institute_id' . '=' . $this->tables['groups'] . '.id')
			->get($this->tables['users_groups']);
	}

	public function get_user_group_type($id = FALSE)
	{
		$this->trigger_events('get_users_group');
	...
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		foreach ($group_ids as $group_id) 
		{
            ...
            $checkUserExist = $this->db->where(array("user_id"=>$user_id))->get("users_groups")->row();
			if(empty($checkUserExist)){
                $hos_data['user_id'] = $user_id;
                $hos_data['group_id'] = $group_id;
                $hos_data['institute_id'] = NULL;
            } else {
                $hos_data['user_id'] = $group_id;
                $hos_data['group_id'] = NULL;
                $hos_data['institute_id'] = $group_id;
            }
			if ($this->db->insert('users_groups', $hos_data)) {
...
public function deleteGroup($id)
	...
	public function create_group_main($group_name = '', $first_initial = '', $last_initial = '', $group_description = '', $group_type = '', $group_info = '', $group_lab_number_format = '', $type_cate = 'category', $parent_group_type="")
	{
		...
		$data = ['name' => $group_name, 'description' => $group_description, 'first_initial' => $first_initial, 'last_initial' => $last_initial, 'information' => $group_info, 'group_type' => $group_type, 'lab_mask' => $group_lab_number_format, 'type_cate' => $type_cate, 'parent_group_type' => $parent_group_type];
	...
	/**
	 * @ Custom Function For Getting Group Type From Group table
	 * 
	 * 16 Nov 2016
	 */
	...
	{
		if (!empty($group_id)) {
			$query = $this->db->query("SELECT `groups`.`group_type`, `groups`.`id` FROM `groups` WHERE `groups`.`id` = $group_id");
			return $query->result();
		}
	}

	/**
	 * Added by: Adeel Yaqoob
	 * @ Custom Function For Updating user(s) password as Admin
	 *
	 * 16 Sep 2020
	 */
    ...
    /**
     * Added by: Shariq Ali
     * @ Custom Function For Updating user(s) password expiry
     *
     * 20 May 2021
     */
    public function update_password_expiry($expiry, $user_id)
    {
        if (!empty($expiry && $user_id)) {
    ...
    public function get_institute_users($institute_id)
    {
        $query = $this->db->query("SELECT AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone, AES_DECRYPT(last_name, '" . DATA_KEY . "') AS last_name, AES_DECRYPT(first_name, '" . DATA_KEY . "') AS first_name, AES_DECRYPT(email, '" . DATA_KEY . "') AS email, users.id as user_id, AES_DECRYPT(username, '" . DATA_KEY . "') AS username  FROM users_groups JOIN users on users_groups.user_id=users.id WHERE users_groups.institute_id='$institute_id'");
    ...
    public function get_institute_name($institute_id)
    {
	 ...
	 public function get_auto_increment_unique($table_name,$field)
    {
		//print "SELECT Max($field) as uni_number from $table_name";
        $query = $this->db->query("SELECT Max($field) as uni_number from $table_name"); 
		$result = $query->result(); 
		//print $result;     
		foreach ($result as $row)
		{			
			 $uniq_id=str_pad(($row->uni_number+1),5, "0", STR_PAD_LEFT);
		}		
        return $uniq_id;
		//exit;
    }
