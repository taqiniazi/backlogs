...
Class Doctor_model extends CI_Model
{
    ...
    public function display_published_record_all($year, $recent = '', $flag_type = '', $sort_authorize = '', $urgency_type = '', $row_color_code = '', $publish)
    {
        $this->get_datatables_record_query_all($year, $recent, $flag_type, $sort_authorize, $urgency_type, $row_color_code, $publish);
        if ($_POST['length'] != -1) 
		{
            $this->db->limit(intval(5), $_POST['start']);
        }
        $query = $this->db->get();
    //    echo $this->db->last_query(); 
		// exit;
    ...
    public function get_datatables_record_query_all($year, $recent = '', $flag_type = '', $sort_authorize = '', $urgency_type = '', $row_color_code = '', $publish)
    {
		...
		{         
            ...
            $this->db->join('billing_data', 'billing_data.id = request.billing_code_ids', 'LEFT');
    ...
    public function record_count_filtered_all($year, $recent = '', $flag_type = '', $sort_authorize = '', $urgency_type = '', $row_color_code = '', $publish = '')
    {
        $this->get_datatables_record_query_all($year, $recent, $flag_type, $sort_authorize, $urgency_type = '', $row_color_code = '', $publish);
        $query = $this->db->get();
...
//        echo $this->db->last_query(); exit;
    ...
    public function get_next_lab_record($lab_id, $status, $request_id)
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        ...
        $labIdStr = $this->getLabIdsFromUser($lab_id);
        $labIds = (!empty($labIdStr)) ? $labIdStr : '0';
        $filter .= " AND request.lab_id IN ($labIds) AND request.request_code_status = '".$status."' AND request.uralensis_request_id != ".$request_id;
        ...
        $query = $this->db->query("SELECT *, CONCAT(AES_DECRYPT(users.first_name, '" . DATA_KEY . "'),' ' ,AES_DECRYPT(users.last_name, '" . DATA_KEY . "')) AS added_by, tbl_courier.courier_no as courier_number, count(DISTINCT(specimen.specimen_id)) as speciman_no FROM request
            ...
            WHERE request.uralensis_request_id = request_assignee.request_id
            AND request.specimen_publish_status = 0
            AND request.supplementary_review_status = 'false' $filter GROUP BY request.uralensis_request_id ORDER BY RAND() LIMIT 1");
        /*AND request.request_add_user = $lab_id */
        // echo $this->db->last_query();exit;
        return $query->row();
    }
    ...
    public function lab_record_counts_by_status($lab_id, $filter='')
    {
        ...
        $query = $this->db->query("SELECT count(request.uralensis_request_id) as request_status_count,request_code_status  FROM request
            INNER JOIN request_assignee                     
    ...
    public function doctor_record_detail($id)
    {
        ...
        $query = $this->db->query("SELECT users.id,AES_DECRYPT(users.phone, '" . DATA_KEY . "') AS phone,
        AES_DECRYPT(company, '" . DATA_KEY . "') AS company,
        ...
        request_datetime, publish_datetime,
        publish_datetime_modified, emis_number,patients.nhs_number,patients.city,patients.state,patients.post_code,patients.address_1, lab_number, hos_number, sur_name,
        f_name, request.dob, age, lab_id, lab_name, date_received_bylab, 
        data_processed_bylab, date_sent_touralensis,
        date_rec_by_doctor, request.gender, clrk, dermatological_surgeon, 
        date_taken, urgent, hsc, report_urgency, cl_detail, specimen_id,
        request.status, specimen_update_status, specimen_publish_status, 
        further_work_status, supplementary_report_status, supplementary_review_status, 
        report_status, publish_status, hospital_group_id, additional_data_state,
        ...
        mdt_case_assignee_username, mdt_case_msg, mdt_case_msg_timestamp, 
        mdt_case_add_to_report_status, mdt_outcome_text, fw_levels, fw_immunos,
        fw_imf, special_notes,patients.phone as p_phone, special_notes_date, record_secretary_id, 
        record_assign_sec_time, record_secretary_status, secretary_record_edit_status,
        cases_category, cost_codes, flag_status, authorize_status, request_add_user, 
        request_add_user_timestamp, clinic_ref_number, clinic_request_form, 
        ...
        INNER JOIN users ON request_assignee.user_id = users.id
        INNER JOIN patients ON patients.id = request.patient_id
        WHERE request.uralensis_request_id = $id ");
        $session_data = array(
            'id' => $id,
            'doctor_id' => $user_id
    ...
    public function doctor_record_detail_specimen($id)
    {
        ...
        $sql = "SELECT users.id,AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone,
                ...
                request.status, request.billing_code_ids, specimen_update_status, specimen_publish_status, 
                further_work_status, supplementary_report_status, supplementary_review_status, 
                report_status, publish_status, hospital_group_id, additional_data_state,
                comment_section, comment_section_date, teaching_case, mdt_case,
                mdt_case_status, mdt_list_id, mdt_specimen_status, 
                ...
                specimen.specimen_id, specimen_clinical_history, specimen_comment_history, specimen_site,
        ...
        foreach ($resArr as $row){
            $row->bill_code_arr = $this->db->select('bd.id, bd.bill_code')->from('billing_data as bd')->join('specimen_type as st', 'st.spec_type_id = bd.specimen_type_id', 'left')->get()->result_array();
    ...
    public function teaching_cases($category_id)
    {
        ...
        AND uralensis_teach_mdt_cats.ura_tec_mdt_id = $category_id");
    ...
    public function display_further_work_model_requested_new()
    {
       ...
       /* $query = $this->db->query("SELECT further_work.fw_id,further_work.furtherwork_date,further_work.fw_status, 
            ...
            FROM further_work
                ...
                WHERE ".$user_where." request.uralensis_request_id = further_work.request_id
         ...
         AND users.id = $user_id */
									...
									$request_t=$this->input->get('fw_page');
                                    if($request_t == '') $request_t = "requested";
									$user_where = "further_work.fw_status = '$request_t' ";
									
									
									
									
		 
// $query = $this->db->query("select requester.id, further_work.fw_id,further_work.furtherwork_date,further_work.request_comment,further_work.fw_status, 
//                                     request.uralensis_request_id as request_id,   
//                                     request.lab_number,
//                                     request.nhs_number,
//                                     request.request_datetime,
...
//                                     request.data_processed_bylab,
//                                     request.date_received_bylab,
//                                     request.publish_datetime,fwd.test_id,
//                                     lt.name as test_name,CONCAT(p1.first_name,' ' ,p1.last_name) AS patient_name,
//                                     sp.block_no as further_block_no, 
//                                     sp.id as further_status_id,
//                                     further_work.fw_status as further_status,
//                                     fwd.test_id,fwd.block_id,request.date_rec_by_doctor FROM further_work_detail fwd 
//                                     LEFT JOIN further_work ON fwd.further_work_id=further_work.fw_id 
//                                     LEFT join request on further_work.request_id =request.uralensis_request_id 
//                                     LEFT JOIN users as requester ON requester.id=request.request_add_user
//                                     LEFT JOIN laboratory_tests as lt ON lt.id=fwd.test_id 
//                                     LEFT JOIN patients as p1 ON p1.id=request.patient_id
//                                     INNER JOIN specimen_blocks as sp ON sp.specimen_id=fwd.block_id AND sp.specimen_no = fwd.test_id
//                                     where ".$user_where."
//                                     group by fwd.id");

        $query = $this->db->query("SELECT count(fwd.id) as testCount, fw.*, requester.id,request.uralensis_request_id as request_id, request.lab_number, request.nhs_number, request.request_datetime, request.hospital_group_id, request.data_processed_bylab, request.date_received_bylab, request.publish_datetime,request.date_rec_by_doctor,CONCAT(p1.first_name,' ' ,p1.last_name) AS patient_name FROM further_work fw
        LEFT JOIN further_work_detail as fwd on fwd.further_work_id = fw.fw_id
        LEFT JOIN request on request.uralensis_request_id = fw.request_id
        LEFT JOIN users as requester ON requester.id=request.request_add_user
        LEFT JOIN patients as p1 ON p1.id=request.patient_id GROUP BY fw.fw_id ORDER BY fw.furtherwork_date DESC");
    ...
    public function display_further_work_model_completed()
    {
        ...
        $labIds = (!empty($labIdStr)) ? $labIdStr : '0';
        $filter = " AND request.lab_id IN ($labIds)";   
        $user_where = "further_work.fw_status = 'complete' ";
        // $query = $this->db->query("SELECT further_work.fw_id, 
        //                             request.uralensis_request_id as request_id,
        //                             requester.id,
        //                             request.serial_number,
...
$query = $this->db->query("select requester.id, further_work.fw_id,further_work.furtherwork_date,further_work.request_comment,further_work.fw_status, 
    ...
    public function display_hospitals_list_with_org($hospitallist = NULL)
    {
        ...
        if (!empty($hospitallist))
        {
            $hoslist = implode(",", $hospitallist);
            if($hoslist != ''){
                $query = $this->db->query("SELECT groups.*, hi.site_identifier, hi.identifier FROM `groups` LEFT JOIN `hospital_information` hi on hi.group_id=groups.id WHERE groups.group_type = 'H' AND groups.id IN(" . $hoslist . ")");
            }else{
...
//        echo $this->db->last_query(); exit;
    ...
    public function specimen_block_list($id){
        ...
        $sql = "SELECT block.block_no, block.specimen_id FROM specimen INNER JOIN specimen_blocks block on block.specimen_id=specimen.specimen_id AND specimen.request_id = $id GROUP BY block_no ORDER BY id ASC";
        $query = $this->db->query($sql);
        return $query->result();
    }
...
//                ";
    ...
    public function get_weekly_request($pathologist_id, $fdate, $tdate)
    {
        ...
        $query = $this->db->select('AES_DECRYPT(u.first_name, "' . DATA_KEY . '") as first_name, AES_DECRYPT(u.last_name, "' . DATA_KEY . '") as last_name, date_format(rq.request_datetime, "%d %M %Y <br> %h:%i %p") as request_date, count(rq.uralensis_request_id) as request_count, rq_as.user_id, sum(if(rq.specimen_publish_status = 1, 1, 0)) as published, sum(if(rq.specimen_publish_status = 0, 1, 0)) as unpublished, , sum(if(rv.request_id > 1, 1, 0)) as request_viewed')
                ...
                ->where("u.id in (".$ids.")")
                //->where("u.user_type", "D")
                ->group_by('date_format(rq.request_datetime, "%Y-%m-%d"), rq_as.user_id')
...
public function getBarcodePrintInfo($req_id){
        $query = $this->db->select('rq.uralensis_request_id as rq_id, rq.lab_number as lab_no, rq.pci_number as lims_no, concat(rq.f_name, " ", rq.sur_name) as patient, count(DISTINCT rs.req_spec_id) as speciman_no, CONCAT(AES_DECRYPT(patho.first_name, "' . DATA_KEY . '"), " ", AES_DECRYPT(patho.last_name, "' . DATA_KEY . '")) AS pathologist, group_concat(DISTINCT(IF(sp_block.name="", "H&E", sp_block.name))) as test, group_concat(sp_block.block_no) as block_no_list, group_concat(concat(sp_block.name,"_",sp.specimen_id) separator ",") as testids, rq.sur_name as pSurname, group_concat(DISTINCT(sp_block.id)) as all_test_id, rq.lab_id, rq.patient_id, br.id as br_id, barcode_image as barcode_img, pt.hospital_id, group_concat(DISTINCT(sp.specimen_id)) as sp_id ')
            ->join('request_assignee rq_as', 'rq_as.request_id = rq.uralensis_request_id', 'inner')
            ->join('users patho', 'patho.id = rq_as.user_id', 'left')
            ->join('users u', 'rq.request_add_user = u.id', 'left')
            ->join('tbl_courier cr', 'cr.id=rq.emis_number', 'left')
		    ->join('patients pt', 'pt.id = rq.patient_id', 'inner')
		    ->join('specimen sp', 'sp.request_id = rq.uralensis_request_id', 'inner')
		    ->join('specimen_blocks sp_block', 'sp_block.specimen_id=sp.specimen_id', 'LEFT')
		    ->join('barcode br', 'br.request_id = rq.uralensis_request_id', 'left')
		    ->join('request_specimen rs', 'rs.rs_request_id = rq.uralensis_request_id', 'left')
            ->where('rq.specimen_publish_status', 0)
            ->where('rq.uralensis_request_id', $req_id)
		    ->where('rq.supplementary_review_status', 'false')
            ->get('request rq')->row();        
        // echo $this->db->last_query();exit;
        return $query;
    }
...
public function task_count(){
        $sql = "SELECT  count(*) as task_count FROM patients where obr IS NOT NULL";
        $query = $this->db->query($sql);
        $result =  $query->row()->task_count;
        return $result;
    }

    public function today_request_count(){
        $date = date("d-m-Y");
        $sql = "SELECT  count(*) as today_request_count FROM request where date_received_bylab = '$date' AND hl7_content <> ''  ";
        $query = $this->db->query($sql);
        $result =  $query->row()->today_request_count;
        return $result;
    }

    public function today_request(){
        $date = date("d-m-Y");
        $sql = "SELECT  count(*) as today_request FROM request where hl7_content <> '' ";
        $query = $this->db->query($sql);
        $result =  $query->row()->today_request;
        return $result;
    }


    /*public function getBarcodePrintInfo($req_id){
        $query = $this->db->select('rq.uralensis_request_id as rq_id, rq.lab_number as lab_no, rq.pci_number as lims_no, concat(rq.f_name, " ", rq.sur_name) as patient, count(DISTINCT rs.req_spec_id) as speciman_no, CONCAT(AES_DECRYPT(patho.first_name, "' . DATA_KEY . '"), " ", AES_DECRYPT(patho.last_name, "' . DATA_KEY . '")) AS pathologist, group_concat(DISTINCT(IF(sp_block.name="", "", sp_block.name))) as test, group_concat(sp_block.block_no) as block_no_list, group_concat(sp_block.name) as testids, rq.sur_name as pSurname, group_concat(DISTINCT(sp_block.id)) as all_test_id, rq.lab_id, rq.patient_id, br.id as br_id, barcode_image as barcode_img, pt.hospital_id')
            ->join('request_assignee rq_as', 'rq_as.request_id = rq.uralensis_request_id', 'inner')
            ->join('users patho', 'patho.id = rq_as.user_id', 'left')
            ->join('users u', 'rq.request_add_user = u.id', 'left')
            ->join('tbl_courier cr', 'cr.id=rq.emis_number', 'left')
