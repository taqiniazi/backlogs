...
Class Barcode_model extends CI_Model 
{
    ...
    public function get_bulk_barcode($ids){
        $this->db->select('br.*, group_concat(DISTINCT(sp_block.name)) as testOld, group_concat(concat(sp_block.name,"_",sp.specimen_id,"_",sp_block.block_no) separator ",") as test,concat(ptnt.first_name," ",ptnt.last_name) as patient_name');
    ...
    public function get_request_bulk_barcode($ids){
        $this->db->select('br.*, group_concat(concat(sp_block.name,"_",sp.specimen_id,"_",sp_block.block_no) separator ",") as test, concat(ptnt.first_name," ",ptnt.last_name) as patient_name, group_concat(DISTINCT(sp.specimen_id)) as sp_id');
        $this->db->from('barcode br');
        $this->db->join('specimen sp', 'sp.request_id = br.request_id', 'inner'); 
		$this->db->join('specimen_blocks sp_block', 'sp_block.specimen_id=sp.specimen_id', 'LEFT');         
        $this->db->join('patients ptnt', 'ptnt.id = br.patient_id', 'left');
