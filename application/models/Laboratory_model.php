    ...
    public function get_record_blocks($request_id){
        ...
        if(!empty($result_specimen)){
            foreach ($result_specimen as $key=>$value){
                ...
                $this->db->group_by('sp_bl.block_no');
...
//        $group_id = $this->ion_auth->get_users_groups($user_id)->row()->id;
    ...
    public function get_lab($id){        
        $this->db->select('gr.description as lab_name, gr.first_initial, gr.last_initial, li.lab_address, li.lab_country, li.lab_city, li.lab_state, li.lab_post_code, li.lab_email, li.site_identifier, li.identifier, li.lab_phone, li.lab_mobile, li.lab_nata, li.lab_fax, li.lab_website, li.group_id, li.logo');
        $this->db->from('laboratory_information li');
        $this->db->join('groups gr', 'gr.id = li.group_id');        
        $this->db->where('gr.group_type', L);
