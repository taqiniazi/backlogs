...
Class Institute_model extends CI_Model
{
    ...
    public function view_final_record()
    {
		...
		AND request.dermatological_surgeon = $user_id
	 ...
	 public function getCourierNo($user_id)
    {
	    ...
	    $user_id = $this->ion_auth->user()->row()->id;
        $group_id = $this->ion_auth->get_users_main_groups($user_id)->row()->id;
		$query = $this->db->query("SELECT * FROM tbl_courier WHERE sender_organization='$group_id' or receiver_organization='$group_id'  order by created_at desc");
        return $query->result();
	}
     ...
     * @return array
     */
    public function institute_record_all($type, $res_type)
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
    ...
    public function status_bar_result_count_published()
    {
        ...
        $result = $this->db->query("SELECT * FROM request INNER JOIN 
        users_request INNER JOIN `groups`
        WHERE users_request.request_id = request.uralensis_request_id
        AND groups.id = users_request.group_id
        AND request.specimen_publish_status = 1
        AND request.hospital_group_id IN ($group_id)"
        );
...
//        $group_id = $this->ion_auth->get_users_main_groups($user_id)->row()->id;
    ...
    public function add_courier($return_courier_id=false)
    {
        ...
        $collected_date = $this->input->post('collection_date');
		 $collected_date = str_replace(' am',':00',$this->input->post('collection_date'));
		 $collected_date = str_replace(' pm',':00',$this->input->post('collection_date'));
        $collection_date = ($collected_date == "" ? date('Y-m-d H:i:s') : date('Y-m-d H:i:s', strtotime($collected_date)));
...
//            $this->db->select("'0' as is_admin");
            ...
            if ($groupType != "HA" OR $groupType != "LA") {
                ...
                if ($userIds != "") {
                    $whereClasue .=" OR (tbl_courier.sender_organization IN ($userIds) OR tbl_courier.receiver_organization IN ($userIds))";
                }
            } else {
                $whereClasue .=" OR (tbl_courier.sender_organization IN ($groupId) OR tbl_courier.receiver_organization IN ($groupId))";
            }
...
//        $hospital_id = $this->ion_auth->get_users_main_groups()->row()->id;
    ...
    public function get_courier_users($institute_id = FALSE)
    {
      ...
      AES_DECRYPT(username, '" . DATA_KEY . "') AS username,AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone,address as address1,usermeta2.meta_value as address2");
