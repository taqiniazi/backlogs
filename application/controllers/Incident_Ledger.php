 ...
 * @version    1.0.0
 */
class Incident_Ledger extends CI_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('Incident_LedgerModel', 'lab_enquiries');
    ...
    public function index()
    {
        ...
        if (count($this->input->post()) > 0) {
            if (isset($_POST['ticket_id']) && $_POST['ticket_id'] != '' && is_numeric($_POST['ticket_id'])) {
                $ticketSaveArr = array(
                    ...
                    'ticket_action' => $this->input->post('ticket_action'),
                    'comment' => $this->input->post('comment'),
                    'ticket_assigned_to' => implode(",", $this->input->post('assignee'))
                );
                ...
                if (!empty($assignee)) {
                    ...
                    $this->db->delete('incident_ledger_assignee');
                    ...
                    foreach ($assignee as $key => $value) {
                        ...
                        $this->db->insert('incident_ledger_assignee', $saveArr);
                    }
                }
                ...
                if (isset($_FILES)) {
                    $config = array(
                        'upload_path' => './uploads/incident_ledger',
                ...
                redirect('Incident_Ledger/', 'refresh');
        ...
        $dataSet['category_list'] = $this->db->get_where('incident_ledger_category', ['created_by' => $user_id])->result_array();
        ...
        $data['javascripts'] = array(
            ...
            'js/incident_ledger/incident_ledger.js'
        );

        $this->load->view('templates/header-new', $data);
        $this->load->view('incident_ledger/ticket', $dataSet);
    ...
    function changePriority($ticketID, $priority)
    {
        if (isset($ticketID) && $ticketID != '0' && $ticketID != '') {
            ...
            $this->db->update('incident_ledger');
            $this->session->set_flashdata('inserted', TRUE);
    ...
    function changeStatus($ticketID, $status)
    {
        if (isset($ticketID) && $ticketID != '0' && $ticketID != '') {
            ...
            if($status == 'cancelled') redirect('Document_List/add_improvment_corrective_action_register/?ticket='.$ticketID, 'refresh');
            else redirect('Incident_Ledger/', 'refresh');
        } else {
            $this->session->set_flashdata('error', TRUE);
            $this->session->set_flashdata('tckSuccessMsg', 'Invalid Ticket, Please Try Again.');
            redirect('Incident_Ledger/', 'refresh');
    ...
    function report_incident()
    {
        ...
        if (count($this->input->post()) > 0) {
            if (isset($_POST['ticket_id']) && $_POST['ticket_id'] != '' && is_numeric($_POST['ticket_id'])) {
                ...
                if (!empty($assignee)) {
                    ...
                    $this->db->where('assignee_ticket_id', $ticketID);
                    if ($this->db->get('incident_ledger_assignee')->num_rows() > 0) {
                        $this->db->where('assignee_ticket_id', $ticketID);
                        $this->db->update('incident_ledger_assignee', array('assignee_id' => $assignee));
                    } else {
                        $this->db->insert('incident_ledger_assignee', array('assignee_ticket_id' => $ticketID, 'assignee_id' => $assignee));
                    }
                    //                    $this->_notify_assignee($assignee, $ticketID);
                }
                ...
                $this->session->set_flashdata('inserted', TRUE);
                $this->session->set_flashdata('tckSuccessMsg', 'Incident Reported...');
                redirect('Incident_Ledger/report_incident/', 'refresh');
            } elseif (isset($_POST['save_type']) && $_POST['save_type'] != '' && $_POST['save_type'] == 'add') {
                //---------- SAVE START----------------
                $errorControl = array();
                $errorMsgs = array();
        ...
        $this->load->view('templates/header-new', $data);
        $this->load->view('incident_ledger/report-ticket', $dataSet);
        $this->load->view('templates/footer-new', $data);
    }
    ...
    function delete($ticketID)
    {
        if (isset($ticketID) && $ticketID != '0' && $ticketID != '' && is_numeric($ticketID)) {
            ...
            $this->db->where('ticket_id ', $ticketID);
            $this->db->delete('incident_ledger');
            $this->session->set_flashdata('inserted', TRUE);
    ...
    public function editTicket()
    {
        ...
        if (!empty($ticketID) && $ticketID != '0' && $ticketID != '' && is_numeric($ticketID)) {
            ...
            $assignedData = $this->lab_enquiries->getTicketAssignee($ticketID);
            foreach ($assignedData as $value) {
                $ticketData[] = $value['id'];
            }
            $dataSet['ticketAssignee'] = $ticketData;
            $dataSet['isUserLab'] = false;
            ...
            $this->load->view('incident_ledger/ticket_edit', $dataSet);
        } else {
    ...
    public function viewTicket($ticketID)
    {
        ...
        if (!empty($ticketID) && $ticketID != '0' && $ticketID != '' && is_numeric($ticketID)) {
            ...
            $this->load->view('incident_ledger/ticket_view', $dataSet);
    ...
    public function deleteAttachment($attachmentID)
    {
        ...
        if (isset($attachmentID) && $attachmentID != '0' && $attachmentID != '' && is_numeric($attachmentID)) {
            ...
            $attachmentRES = $this->db->get('incident_ledger_attachment');
            if ($attachmentRES->num_rows() > 0) {
                $attachData = $attachmentRES->row();
                $filePath = "./uploads/incident_ledger/" . $attachData->attachment_path;
                unset($filePath);

                $this->db->where('attachment_id', $attachmentID);
                $this->db->delete('incident_ledger_attachment');
    ...
    public function deleteComment($commentID, $ticketID)
    {
        if (isset($commentID) && $commentID != '0' && $commentID != '' && is_numeric($commentID)) {
            ...
            $this->db->update('incident_ledger_comments');
        }
        redirect('Incident_Ledger/viewTicket/' . $ticketID, 'refresh');
    }
    ...
    public function addComment()
    {
        ...
        if (isset($ticketID) && $ticketID != '0' && $ticketID != '' && is_numeric($ticketID)) {
            if ($comment != '') {
                ...
                $this->db->insert('incident_ledger_comments', $saveArr);
            }
        }
        redirect('Incident_Ledger/viewTicket/' . $ticketID, 'refresh');
    }
    ...
    public function manage_category()
    {
        ...
        if ($this->input->post()) {
            ...
            if (empty($id)) {
                $count = $this->db->get_where('incident_ledger_category', ['title' => $title])->num_rows();
            } else {
                $count = $this->db->get_where('incident_ledger_category', ['title' => $title, 'id!=' => $id])->num_rows();
            }

            if ($count == 0) {
                $flag = false;
                if (empty($id)) {
                    $this->db->insert('incident_ledger_category', ['title' => $title, 'created_by' => $userID, 'updated_by' => $userID]);
                    if ($this->db->insert_id() > 0) {
                        $flag = true;
                        $msgText = 'Category added successfully.';
                    }
                } else {
                    $this->db->update('incident_ledger_category', ['title' => $title, 'updated_by' => $userID], ['id' => $id]);
                    $flag = true;
    ...
    public function delete_category($id)
    {
        ...
        $flag = false;
        if ($id > 0) {
            $cnt = $this->db->get_where('incident_ledger_category', ['id' => $id])->num_rows();
            if ($cnt > 0) {
                $res = $this->db->delete('incident_ledger_category', ['id' => $id]);
                if ($res) {
                    $flag = true;
                }
            }
