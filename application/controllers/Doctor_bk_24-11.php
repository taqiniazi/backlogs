    ...
    public function display_published_reports_ajax_processing()
    {
        ...
        foreach ($list as $record) 
		{
            ...
            if ($record->specimen_update_status == 1 && $record->specimen_publish_status == 1) {
                $record_published = '<a data-toggle="tooltip" data-placement="top" title="' . $record->serial_number . ' Record Has Been Published Or Completed." href="' . site_url() . '/Institute/view_singlerecord/' . $record->uralensis_request_id . '"><img src="' . base_url('assets/img/completed.png') . '"></a>';
            }
            $supply_record = '';
    ...
    public function delete_test_data($id, $testId){
        if(!empty($testId)){
            ...
            if(isset($row)){
                ...
                $this->db->delete('specimen_blocks', $whereArr);
				 $this->db->delete('further_work', $whereArr2);
...
public function updatePatientsRecord()
    ...
    {
        $result['status'] = "fail";
        if($this->input->post() && $this->input->post('changeId')){
