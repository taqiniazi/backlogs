...
class Institute extends CI_Controller 
{
  ...
  private $group_type = "";
     ...
     * @return void
     */
    public function reports($type='all') 
	  {
      if (!$this->ion_auth->logged_in()) {
        redirect('auth/login', 'refresh');
      }
      ...
      $data["query"] = $this->Institute_model->institute_record_all($type, 'result');
      $data['type'] = $type;
      $this->load->view('institute/inc/header');
      $this->load->view('institute/institute_published', $data);
      $this->load->view('institute/inc/footer-new');
    }
    ...
    /**
     * all Report Ajax Processing
     * using datatables
     *
     * @return void
     */
    public function type_wise_report() {
      $type = trim($this->input->post('type'));      
      if (!$this->ion_auth->logged_in()) {
        redirect('auth/login', 'refresh');
      }
      $list = $this->Institute_model->institute_record_all($type, 'result');
      $data = array();
      $flag_count = 11;
      foreach ($list as $record) {
        ...
        if ($record->specimen_publish_status == 1) {
          $publish_status = '<a target="_blank" href="' . site_url('doctor/generate_report/' . $record->uralensis_request_id) . '"><img src="' . base_url('assets/img/view.png') . '">&nbsp;</a>';
        }
        $publish_status_download = 'Not Published';
        if ($record->specimen_publish_status == 1) {
          $publish_status_download = '';
        }
        $f_initial = '';
        $l_initial = '';
        ...
        $row[] = '<a target="_blank" href="' . site_url() . '/doctor/generate_report/' . $record->uralensis_request_id . '"><img src="' . base_url('assets/img/adobe.png') . '" /></i></a>';
//        $row[] = '';
        $row[] = $assign_status;
        //$row[] = $publish_status;
        //$row[] = $publish_status_download;
        //$row[] = '<a href="' . site_url() . '/doctor/generate_report/' . $record->uralensis_request_id . '"><img src="' . base_url('assets/img/adobe.png') . '" />&nbsp; Docs</a>';
        $data[] = $row;
        $flag_count++;
      }
      $output = array(
        "draw" => $_POST['draw'],
        "recordsTotal" => intval($this->Institute_model->institute_record_all($type, 'count')),
        "recordsFiltered" => intval($this->Institute_model->institute_record_all($type, 'count')),
        "data" => $data,        
      );
    ...
    public function published_reports_ajax_load() {
      ...
      $list = $this->Institute_model->institute_record_published();
      $data = array();
      $flag_count = 11;
      foreach ($list as $record) {
        $row_code = '';
    ...
    public function getTemplates() {
      ...
      $user_id = $this->ion_auth->user()->row()->id;
      $templatename = $this->input->post('track_template_name');
        $templateArray = $this->getTemplateDetails($user_id, $templatename);
    ...
    private function getTemplateDetails($user_id, $templatename){
        $templateArray = [];
...
//    }
    ...
    public function bookingin() 
    {
     if (!$this->ion_auth->logged_in())  
      ...
      $h_data = array('styles' => array('css/institute/bookingin.css'));
      $f_data = array('javascripts' => array('js/typeahead.jquery.js', 'js/institute/bookinging.js'));
...
//        print_r($lab_names['lab_names']); exit; 
  ...
  if (is_array($valid_session_ids) && !empty($valid_session_ids)) {
   // $session_rec_data['session_data'] = $this->Institute_model->get_all_session_records($valid_session_ids);
        ...
        // $result_data['hospitals'] = $this->Institute_model->fetch_all_hospitals();
		  ...
		  {
            $result_data['hospitals'] = $this->db->get_where('groups', ['id' => $this->group_id])->result_array();
          }
		  else
		  {
			 $result_data['hospitals'] = $this->Institute_model->fetch_all_hospitals(); 
	...
	function get_speciality_snomed($ids=false)
    {
        ...
        $sql = "SELECT * from speciality_snomed_codes where category_id='$ids' and lab_id='".$group_id."'";
        $query = $this->db->query($sql);
  ...
  function getHospitalclinicians($hospitalId)
    {
        $sql = "SELECT groups.id,groups.description
                FROM hospital_group 
                JOIN groups on hospital_group.group_id=groups.id 
                WHERE hospital_group.hospital_id='$hospitalId' AND groups.group_type='C'";
...
function get_department_list($deppart_list=''){
    ...
    public function save_new_track_temp_data() 
    {
      ...
      if (!empty($_POST)) 
	  {
		...
		$lab_no = $this->input->post('bookingin_lab_id');
        ...
        if ($this->input->post('edit_mod') == "add" && empty($this->input->post('template_id'))) {
          ...
          $temp_data = array(
            ...
            'temp_report_urgency' => !empty($report_urgency) ? $report_urgency : 'Routine',
            'temp_skin_type' => !empty($specimen_type) ? $specimen_type : '',
            'temp_input_name' => !empty($template_name) ? $template_name : '',
            'specimen_no' => !empty($specimen_no) ? $specimen_no : '1',
			'lab_no' => !empty($lab_no) ? $lab_no : '',
          ...
          $json['type'] = 'success';
          $json['msg'] = 'Order template created successfully.';
          echo json_encode($json);
          die;
        } else {
                //Prepare Data for update
          ...
          $json['type'] = 'success';
          $json['msg'] = 'Order template updated successfully.';
        ...
        if (isset($_POST) && $_POST['search_type'] === 'only_search') {
		  ...
		  if (!$this->ion_auth->logged_in()) 
		  {
		   ...
		   {$lab_no=$lab_no;}
            ...
            $serial_query = $this->db->query("SELECT uralensis_request_id FROM request WHERE uralensis_request_id = $req_id_before_insert");
            if ($serial_query->num_rows() > 0) 
			{
              $row = $serial_query->row();
              //$last_inserted_serial_number = $row->serial_number;
              ...
              if ($keyParts[1] == date('y')) 
			  {
                $key = $keyParts[1] . $keyParts[0] . "0300" . ($keyParts[2] + 1);
              } 
			  else 
			  {
			...
			{
				$lab_name ='FNQH Pathology Pty Ltd';
			}
            ...
            $record_edit_status = array(
              ...
              'emis_number' => $patient['medicare_card_no'],
              'lab_number' => $key,
              'dob' => $patient['dob'],
              'date_received_bylab' => date('Y-m-d'),
              'date_sent_touralensis' => date('Y-m-d'),
              'rec_by_doc_date' => date('Y-m-d'),
              'clrk' => 'no',              
			  'ura_barcode_no' => $key,
              'pci_number' => 'FN'.$key,
			...
			$blocks_data = array('specimen_id' => $specimen_id, 'specimen_no' => 1,'block_no' => '1A','description' => 'H&E L1');
...
public function batch_record()
{
		  ...
		  if (!$this->ion_auth->logged_in()) 
		  {
            ...
            $this->load->model('PatientModel', 'patient');
            $this->load->model('DepartmentModel', 'department');            			
			$tem_id=$this->input->post('template_id');
			$num_Spno=1;
           ...
           $serial_query = $this->db->query("SELECT uralensis_request_id FROM request WHERE uralensis_request_id = $req_id_before_insert");
            if ($serial_query->num_rows() > 0) 
			{
              ...
              $key = date('y').'FN10'.'00'.'1';
            } else {
              $key = date('y').'FN10'.'00'.'0';
            }
            $initial = '';
            if (!empty($patient['first_name']) && strlen($patient['first_name']) > 0) {
              $initial .= $patient['first_name'][0];
...
$json['type'] = 'success';
$json['msg'] = 'Case created successfully.';
$json['redirect_url'] = base_url('doctor/doctor_record_detail_old/').$record_last_id;
echo json_encode($json);
die;			
}
    ...
    public function search_and_add_barcode_record() 
    {
        ...
        if (isset($_POST) && $_POST['search_type'] === 'only_search') 
        {
          ...
          if (!empty($query)) 
		  {
            ...
            $patient_search="<select class='selecte2'><option></option></select>";
          ...
          //if we want to add ulnumber
            ...
            if (!empty($template_data)) 
            {
              $hospital_id = $template_data['hospital_id'];
              $clinic_user_id = $template_data['temp_clinic_user'];
              $pathologist_id = $template_data['temp_pathologist'];
              $lab_id = $template_data['temp_lab_name'];
              $urgency = $template_data['temp_report_urgency'];
              $speci_type = $template_data['temp_skin_type'];
              $specimen_no=$template_data['specimen_no'];
            }
            $user_id = $this->ion_auth->user()->row()->id;
            $get_lab_name = $this->db->select('name')->where('id', $lab_id)->get('groups')->row_array();
                ...
                // Institute data validated
                    ...
                    // redirect them back to the admin page
                        $info_data = array(
                          ...
                          'lab_phone' => $this->input->post('laboratory_number'),
                          'lab_mobile' => $this->input->post('laboratory_mobile_num'),
                          'lab_nata' => ($this->input->post('laboratory_nata_num') ? $this->input->post('laboratory_nata_num') : ''),
                          'lab_fax' => $this->input->post('laboratory_fax'),
                          'lab_website' => $this->input->post('laboratory_website'),
...
public function delete_new_track_temp_data($ids)
{
  ...
  public function saveHospitalData() {
    ...
    public function get_pathologist_and_clinician()
    {
        ...
        {
            $hospital_id = $this->input->post('hospital_id');
            $pathologyArr = $this->Institute_model->get_doctors($hospital_id);
            $clinicianArr = $this->Institute_model->get_clinic($hospital_id);

            $json['status'] = "success";
            $json['message'] = "";
            $json['data'] = ['pathology' => $pathologyArr, 'clinician' => $clinicianArr];
        }
        echo json_encode($json); exit;
    }

    public function addBookingDataFromCsv() 
	{
        $count = 0;
        if($_REQUEST) 
		{
            $user_id = $this->ion_auth->user()->row()->id;
            $templateID = $this->input->post('track_template_name');
            $templateArray = $this->getTemplateDetails($user_id, $templateID);
			$template_data = $this->db->where('ura_rec_temp_id', $templateID)->get('uralensis_record_track_template')->row_array();
            ...
            if (!empty($template_data)) 
			{
              ...
              $clinic_user_id = $template_data['temp_clinic_user'];
              $pathologist_id = $template_data['temp_pathologist'];
              $labName = $template_data['temp_lab_name'];
              $urgency = $template_data['temp_report_urgency'];
              $speci_type = $template_data['temp_skin_type'];
			  $courier_no = $template_data['courier_no'];
              $specimen_no = $template_data['specimen_no'];
			  
            }
            $user_id = $this->ion_auth->user()->row()->id;
            $get_lab_name = $this->db->select('name')->where('id', $labId)->get('groups')->row_array();
            ...
            if ($_FILES["UploadCSV"]["size"] > 0)
            {
                ...
                $hospital_row = $this->db->select('name')->where('id', $hospital_id)->get('groups')->row_array();
                $hospital_id = $hospital_id;
				
				$get_hos_fname = $this->db->select('first_initial')->where('id', $hospital_id)->get('groups')->row_array();
			    $get_hos_lname = $this->db->select('last_initial')->where('id', $hospital_id)->get('groups')->row_array();
				...
				{
                    $last_d = str_pad($j, 4, "0", STR_PAD_LEFT);
                    $batch_no = 'BN'.date('y') . '' . $last_d;
					$lab_no=date('y')."FN0".$last_d;
                }
		        else {
                    $last_d = str_pad($j, 4, "0", STR_PAD_LEFT);
					$lab_no=date('y')."FN0".$last_d;
                    $batch_no = "BN".$lab_no;
					//$lab_no="FN22-".$last_d."-".$f_initial.$l_initial;
                }
                while (($column = fgetcsv($file, 10000, ",")) !== FALSE)
                {
                    if ($i >= 1) 
					{
                        ...
                        if (isset($column[0])) {
                            $request['pci_number'] = $column[0];
							$request['lab_number'] = $column[0];
                        }
                        if (isset($column[1])) 
						{
                            $patient = explode(' ', $column[1], 2);
                            $request['f_name'] = $patient[0];
                            $request['sur_name'] = $patient[1];
                            $request['patient_initial'] = substr($patient[0], 0, 1) . substr($patient[1], 0, 1);

                            $patientArr['first_name'] = $patient[0];
                            $patientArr['last_name'] = $patient[1];
                            $patient_id = $this->db->get_where('patients', ['first_name'=>$patient[0], 'last_name'=>$patient[1]])->row()->id;
                        }
                        if (isset($column[2])) 
						{
                            $dob = date('Y-m-d', strtotime($column[2]));
                        ...
                        if (isset($column[4]))
						{
                            $request['request_type'] = $column[4];
							$specimen_type = $column[4];
                        }
                        if (isset($column[5])) 
						{
                            $request['speciman_no'] = $column[5];
							//$specimen_no = $column[5];
                        }
                        if (isset($column[6])) 
						{
                            $request['request_datetime'] = date('Y-m-d H:i:s', strtotime($column[6]));;
                        }

                        $request['lab_id'] = $labId;
                        $request['lab_name'] = "FNQH";
						$request['emis_number'] = $courier_no;
                        $request['template_id'] = $templateID;
                        $request['record_batch_id'] = $batch_no;
                        $request['hospital_group_id'] = $hospital_id;
						
						$request['date_received_bylab'] = date('Y-m-d H:i:s');
						$request['date_taken'] = date('Y-m-d H:i:s');						
                        $request['request_add_user'] = $this->ion_auth->user()->row()->id;
                        $request['request_assign_status'] = '1';
                        $request['clinic_ref_number'] = '';
                        $request['clinic_request_form'] = '';
                        $request['speciality_group_id'] = $templateArray['speciality'];
                        
						$patientArr['hospital_id'] = $hospital_id;
                        $patientArr['created_at'] = date('Y-m-d H:i:s');
                        $patientArr['updated_at'] = date('Y-m-d H:i:s');
                        if(!isset($patient_id))
						{
            ...
            if ($serial_query->num_rows() > 0) 
			{
              ...
              if ($keyParts[1] == date('y')) 
			  {
                $key = $keyParts[1] . $keyParts[0] . "000" . ($keyParts[2] + 1);
              } else {
              ...
              $key = date('y').'FN'.'00'.'1';
                        ...
                        $checkDuplicateReq = $this->db->get_where('request',[
                            ...
                            'record_batch_id'   => $request['record_batch_id'],
                            'hospital_group_id' => $request['hospital_group_id'],
                            'speciality_group_id' => $request['speciality_group_id'],
							'date_taken' => date('Y-m-d'),
							'date_received_bylab' => date('Y-m-d'),
							'date_sent_touralensis' => date('Y-m-d')
                        ]);
                        if($checkDuplicateReq->num_rows() == 0){
                            $res = $this->db->insert('request', $request);
                            $record_last_id = $this->db->insert_id();
                        }else{
                            $record_last_id = $checkDuplicateReq->row()->uralensis_request_id;
                        }
                        ...
                        for ($i = 1; $i <= $specimen_no; $i++) {
                            $specimen = array(
                                'request_id' => $record_last_id,
                                'specimen_site' => '',
                                'specimen_procedure' => '',
                                'specimen_type' => (isset($column[4]) && !empty($column[4])) ? $column[4] : '',
                                'specimen_block' => (isset($column[3]) && !empty($column[3])) ? $column[3] : '',
                                'specimen_slides' => (isset($column[5]) && !empty($column[5])) ? $column[5] : '',
                                'specimen_block_type' => '',
                                'specimen_macroscopic_description' => '',
                                'specimen_diagnosis_description' => '',
                                'specimen_cancer_register' => '',
                                'specimen_rcpath_code' => '',
                                'blockDetail' => (isset($column[3]) && !empty($column[3])) ? $column[3] : '',
                                'slides' => (isset($column[5]) && !empty($column[5])) ? $column[5] : '',
                            );
                            $this->db->insert('specimen', $specimen);
                            $inserted_specimen_id = $this->db->insert_id();
                            if($inserted_specimen_id > 0){
                                $this->db->insert('specimen_blocks', [
                                    'specimen_id'   => $inserted_specimen_id,
                                    'specimen_no'   => 1,
                                    'block_no'      => '1A',
                                    'description'   => 'HE',
                ...
                if($this->db->get_where("request_assignee", $request_assign)->num_rows() == 0){
                    $this->db->insert("request_assignee", $request_assign);
                }
                $assign_request_args = array(
                  'assign_status' => intval(1),
                ...
                $this->session->set_flashdata('success', "Total $count records are import successfully.");
            } else {
                $this->session->set_flashdata('error', 'Something went wrong, Please Try Again..!');
            }
            //redirect('lab/lab_record_list', 'refresh'); return;
        } else {
            $this->session->set_flashdata('error', 'Something went wrong, Please Try Again..!');
        }
        redirect('lab/lab_record_list', 'refresh'); return;
    ...
    /* below functions are moved from doctor to institute */
    public function doctor_record_list()
    {
        ...
        $data['doctor_list'] = $this->Admin_model->get_doctors();
        $data["query"] = $this->Doctor_model->hospital_record_list($this->group_id, $filter);
        $data['request_slides_id'] = $this->Doctor_model->doctor_record_list_with_slide($doctor_id, $filter);
        $checkhospital = getRecords("group_id", "users_groups_type", array("user_id" => $doctor_id));
        $hospitallist = array();
        foreach ($checkhospital as $rec)
    ...
    public function published_reports_new()
    {
        /* if (!$this->ion_auth->logged_in()) {
             redirect('auth/login', 'refresh');
         }
         $this->mybreadcrumb->add('Dashboard', base_url('index.php/doctor'));
         $this->mybreadcrumb->add('Publish Reports', base_url('index.php/institute/published_reports_new'));
         $breadcrumb['breadcrumbs'] = $this->mybreadcrumb->render();
      ...
      $this->group_id = $group_row->id;
        ...
        $filter = $filter . " AND publish_status = 1";
        $data['doctor_list'] = $this->Admin_model->get_doctors();
        $data["query"] = $this->Doctor_model->hospital_record_list_publish($hospital_id, $filter);
        $data['request_slides_id'] = $this->Doctor_model->doctor_record_list_with_slide($doctor_id, $filter);
        $checkhospital = getRecords("group_id", "users_groups_type", array("user_id" => $doctor_id));
        $hospitallist = array();
        foreach ($checkhospital as $rec)
