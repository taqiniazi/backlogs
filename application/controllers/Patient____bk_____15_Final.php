...
//$pData->pdf_content=$this->db->get_where('request', ['uralensis_request_id' => $request_id])->row()->hl7_content;
        ...
        if ($this->input->post()) {
            ...
            if (isset($id) && !empty($id)) {
                ...
                if (isset($pData)) {
                    ...
                    if (isset($hl7_content) && !empty($hl7_content)) 
					{
                        $pData->hl7_content = $hl7_content;
                        $content = $this->text_file_content_billing($pData);
						...
						else 
						{
                            $this->db->insert('generated_file_message_data', [
                                ...
                                'file_type' => 'HL7',
                                'file_name' => $newFileName,
                                'file_path' => '/' . MDR_Billing . '/',
                                'created_by' => $createdBy
                            ]);
                            $this->db->update('patients', ['obr' => $filePath1], ['id' => $id]);
                            $resArr = ['status' => 'success', 'message' => 'Successfully generate billing file'];
                        }
                    } 
					else 
					{
                        ...
                        $filePath2 = './' . MDR_Billing . "/$newFileName";
                        if (!is_dir('./' . MDR_Billing)) {
                            mkdir('./' . MDR_Billing, 0777, TRUE);
                        }

                        if (!write_file($filePath2, $content['res'])) {
                            $resArr = ['status' => 'error', 'message' => 'Unable to write the file'];
                        } else {
                            $this->db->insert('generated_file_message_data', [
                                'patient_id' => $id,
                            ...
                            $this->db->update('patients', ['obr' => $filePath2], ['id' => $id]);
                            $resArr = ['status' => 'success', 'message' => 'Successfully generate ORM file'];
                        }
                    }
                } else {
                    $resArr = ['status' => 'error', 'message' => 'Patient data not found on database'];
    ...
    private function text_file_content_type1($data){
        ...
        $contentArr[] = "OBX|$obxCnt|FT|$reportType^^AUSLAB||$pdfData||||||F|||$dateTime|^^AUSLAB";
        $resContentTxt = join('', $contentArr);
...
//$res .= rtrim($res_val[2])."\n";
		...
		$res .="OBR|1||07-1234567-FNQH-0^2203.AUSNATA^2203^L|Histology|||$rdate_final|||L|L||".$obx_array[13]."|||".$obx_array[16]."|||||||||||^^^$rdate_final\r\n";
        $case2 .= "OBX|1|FT|Histopathology Report^^AUSLAB||$report||||||F|||$pdate_final|^^AUSLAB";
