...
Class Doctor extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $dashboard_reports_status['upload_docs'] = $this->Doctor_model->get_upload_doc_forms();
...
//        echo '<pre>'; print_r($dashboard_reports_status['doc_case_files']); exit;        
        $dashboard_reports_status['unpublished_stats'] = $this->Doctor_model->dr_dash_unpublished_stats_new($doctor_id);
        //$dashboard_reports_status['unpublished_stats'] = $this->Doctor_model->dr_dash_unpublished_stats($doctor_id);
        // print_r($dashboard_reports_status['unpublished_stats']);  exit();
...
//                Months Not Exists
    ...
    public function upload_docs_form() {
        ...
        if (isset($_FILES['upload_doc']) && $_FILES['upload_doc']['name'] != '') {
            $ref_key = $user_id;
            $upload_doc = $this->do_upload_doctor_files('upload_doc', $ref_key);
            if ($upload_doc === FALSE) {
                $error = array('upload_error' => $this->upload->display_errors());
                $this->session->set_flashdata('upload_error', $error['upload_error']);
                redirect('doctor');
                ...
                $file_path = "doctor_uploads/" . $checklist_file_name;
    ...
    /* Upload Doctor Files */
    public function do_upload_doctor_files($doctor_file_name, $ref_key) {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        $config['upload_path'] = './doctor_uploads/';
        $config['allowed_types'] = 'pdf|doc|xls|xlsx|png|jpeg|jpg';
        $config['max_size'] = 20400;
        $config['overwrite'] = TRUE;
        $new_name = $ref_key . '-' . $_FILES[$doctor_file_name]['name'];
        $config['file_name'] = $new_name;
        $this->load->library('upload', $config);
        $this->upload->initialize($config);
        if (!$this->upload->do_upload($doctor_file_name)) {
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function download_forms($filename) {
        ...
        // read file contents
        $data = file_get_contents(base_url('doctor_uploads/' . $filename));
    ...
    public function doctor_record_list() {
        ...
        if (isset($hospital_id) && !empty($hospital_id) && trim($hospital_id) != '') {
            $hospital_id = trim($hospital_id);
            $data['hospital_id'] = $hospital_id;
            $filter = $filter . " AND hospital_group_id = '$hospital_id'";
        }
        ...
        if (isset($id) && !empty($id)) {            
            ...
            switch ($user_type) {
                ...
                case 'H':
					...
					case 'L':
                    // Check if request is of Hospital
                    $res = $this->db->query("SELECT `lab_id` FROM `request` WHERE `request`.`uralensis_request_id` = $id")->result_array();
                    if (count($res) != 0) 
					{
                        $lab_id = $res[0]['lab_id'];
                        $res = $this->db->query("SELECT * FROM `users_groups` WHERE `user_id` = $doctor_id AND `group_id` = $lab_id")->result_array();
                        if (count($res) == 0) {
                            return redirect('/', 'refresh');
                        }
                    }
 ...
 if($data['request_query'][0]->specimen_publish_status == 1){
               // redirect('/Institute/view_singlerecord/'.$id, 'refresh');
            }
            ...
            switch ($user_type) 
			{
                ...
                case 'H':
                    ...
                    if (count($res) != 0) {
                        $hos_id = $res[0]['lab_id'];
                        $res = $this->db->query("SELECT * FROM `users_groups` WHERE `user_id` = $doctor_id AND `institute_id` = $hos_id")->result_array();
                        if (count($res) == 0) {
                            return redirect('/', 'refresh');
                        }
                    }
        ...
        //  echo "<pre>";print_r($slide_data['slide_data']);exit;
            ...
            $data['request_query'][0]->microscopicList = $this->db->query("SELECT * FROM uralensis_micro_codes")->result();
            $data['request_query'][0]->barcodeInfo = $this->Doctor_model->getBarcodePrintInfo($id);

            $data['list_doctors'] = $this->Admin_model->get_doctors();
        ...
        $data['request_query'][0]->urole = $this->ion_auth->get_users_groups()->row()->name;
		$hospital_users['hos_users'] = $this->Doctor_model->get_hospital_groups();

        /* start lab wise test list */
        $testListArr = [];
        $recordDetailsArr = $this->Doctor_model->getDetailsAgainstRequest($id);
        if($recordDetailsArr && $recordDetailsArr[0]['lab_id'] > 0){
            $labId = $recordDetailsArr[0]['lab_id'];
            $testListArr = getTestAgsinstSubCatCustom($labId);
            // $labId = $recordDetailsArr[0]['lab_id'];
            // foreach (getMainTestCategories() as $mainCatValue) {
            //     $subCategoriesTests = getTestAgsinstSubCat($mainCatValue["id"], $labId);
            //     foreach ($subCategoriesTests as $testValue) {
            //         $testListArr[] = $testValue;
            //     }
            // }
        }
        $data['testListArr'] = $testListArr;
        /* end lab wise test list */


        /*$labListArr = $this->Doctor_model->getLabNamesFromLabGroups();
        $recordDetailsArr = $this->Doctor_model->getDetailsAgainstRequest($id);
        if($recordDetailsArr && $recordDetailsArr[0]['lab_id'] > 0)
        {
            //Get all level 1
            $labId = $recordDetailsArr[0]['lab_id'];
            $getMainTestCategories = getMainTestCategories();
            $testSubCategoryArr = array();
            $i = 0;
            foreach ($getMainTestCategories as $mainCatArray => $mainCatValue) {
                ...
                foreach ($getSubCatAgainstMain as $subValue => $subKey) {
                    ...
                    $t = 0;
                    $subCategoriesTests = getTestAgsinstSubCat($subKey["id"], $labId);
                    foreach ($subCategoriesTests as $testKey => $testValue) {
                        $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_id"] = $testValue["id"];
                        $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_name"] = $testValue["test_id"];
                        $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_desction"] = $testValue["name"];
                        $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["cost"] = $testValue["cost"];
                        $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["sale"] = $testValue["sale"];
                        $t++;
                    }
                    $s++;
                }
            ...
            $data["categoriesArr"] = $testSubCategoryArr;
        }
        pre($data["categoriesArr"]);*/

        $data['categoriesArr'] = getMainTestCategories();
...
//        pre($data);
    ...
    public function generate_report($id)
    {
        ...
        if (isset($id) && !empty($id)) {
			...
			$dID=$data1['query1'][0]->dermatological_surgeon;
			if($dID!='')
			{
				$query_c = $this->db->query("SELECT AES_DECRYPT(users.last_name, '" . DATA_KEY . "') AS last_name,
        AES_DECRYPT(users.first_name, '" . DATA_KEY . "') AS first_name from users where id= $dID");
				$query_r=$query_c->result();
				//print_r($query_r);
				$clinician_name=$query_r[0]->first_name." ".$query_r[0]->last_name;
				//exit;
			}
			else
			{
	    ...
	    public function generate_report2($id)
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        ...
        AES_DECRYPT(users.first_name, '" . DATA_KEY . "') AS first_name from users where id= $dID");
            ...
            $this->load->view('doctor/pdf2', $result);
        }
    }
...
error_reporting(E_ALL);*/
        ...
        if (isset($_POST)) {
            ...
            $clinical_history = $this->input->post('specimen_clinical_history');
            $specimen_comment_history = $this->input->post('specimen_comment_history');
			$comment_history = $this->input->post('comment_history');
			
            if($this->input->post('specimen_microscopic_code')){
                $implode_string = implode(",",$this->input->post('specimen_microscopic_code'));
            }else{
                $implode_string = "";
            }
            ...
            // $implode_string = implode(",",$this->input->post('specimen_microscopic_code'));
            // if(isset($this->input->post('specimen_microscopic_code')) && count($this->input->post('specimen_microscopic_code')) === 0){
            //     // $implode_string = "";
            // }
            // if(count($this->input->post('specimen_microscopic_code'))){
            ...
            $spec = array(
                'specimen_clinical_history' => $clinical_history,
                'specimen_comment_history' => $comment_history,
                ...
                'specimen_microscopic_code' => $implode_string,
            ...
            $allSpecimenUpdateData = [
                'specimen_clinical_history' => $clinical_history, 
				'specimen_comment_history' => $specimen_comment_history,
                'specimen_benign'           => (!empty($specimen_benign) ? $specimen_benign : ''),
                'specimen_atypical'         => (!empty($specimen_atypical) ? $specimen_atypical : ''),
                'specimen_malignant'        => (!empty($specimen_malignant) ? $specimen_malignant : ''),
                'specimen_inflammation'     => (!empty($specimen_inflammation) ? $specimen_inflammation : '')
            ];
                //$this->db->whereIn('specimen_id', $this->input->post('specimen_list_id'))update('specimen', $allSpecimenUpdateData);
            $this->db->where('request_id', $record_id)->update('specimen', $allSpecimenUpdateData);


//            $this->db->where('request_id', $record_id)
//                //->whereIn('specimen_id', $this->input->post('specimen_list_id'))
//                ->update('specimen', ['specimen_clinical_history' => $clinical_history]);

            $this->db->update('request', ['clinical_history' => $clinical_history], ['uralensis_request_id' => $record_id]);
            ...
            if ($check_record == 1) {
                ...
                $existingRow = $this->db->get_where('request_billing_code', $reqWhereArr)->num_rows();*/
                // if(isset($this->input->post('bill_code')[$specimen_id])){

                $billing_type = $this->input->post('billing_type');
                $bill_code_arr = $this->input->post('bill_code');
                $specimen_list_arr = $this->input->post('specimen_list_id');
                $current_user_id = $this->ion_auth->user()->row()->id;

                foreach ($bill_code_arr as $bill_primary_id){
                    foreach ($specimen_list_arr as $new_specimen_id)
					{/*
                        $billData = $this->db->get_where('billing_data', ['id' => $bill_primary_id])->row();

                        $reqWhereArr = [
                            'request_id'    => $record_id,
                            'specimen_id'   => $new_specimen_id,
                            'bill_code'     => $bill_primary_id
                        ];
                        $this->db->delete('request_billing_code', $reqWhereArr);

                        $this->db->insert('request_billing_code', [
                            'billing_type'      => (!empty($billing_type) ? $billing_type : 'by_request'),
                            'bill_code'         => $bill_primary_id,
                            'bill_code_text'    => $billData->bill_code,
                            'bill_description'  => $billData->bill_description,
                ...
                /*for($k=0; $k<count($bill_code_arr); $k++){

                    $bill_primary_id = $this->input->post('bill_code')[$k];

                    foreach ($this->input->post('specimen_list_id') as $new_specimen_id) {
                        $reqWhereArr = [
                            'request_id'    => $record_id,
                            'specimen_id'   => $new_specimen_id,
                            'bill_code'     => $bill_primary_id
                        ...
                        if ($existingRow > 0) {
                            $this->db->where($reqWhereArr)->limit(1)->update('request_billing_code', $reqBillData);
                        } else {
                            $reqBillData['request_id'] = $record_id;
                            $reqBillData['specimen_id'] = $new_specimen_id;
                            $reqBillData['created_by'] = $this->ion_auth->user()->row()->id;
                            $this->db->insert('request_billing_code', $reqBillData);
                        }
                    }
                }*/
                ...
                /*$reqBillData = [
                    'billing_type'      => $this->input->post('billing_type'),
                    'bill_code'         => $this->input->post('bill_code')[$specimen_id],
                    'bill_code_text'    => $this->input->post('bill_code_text')[$specimen_id],
                    'bill_description'  => $this->input->post('bill_description')[$specimen_id],
                    'bill_price'        => $this->input->post('bill_price')[$specimen_id],
                    'updated_by'        => $this->ion_auth->user()->row()->id
                ];
                if($existingRow > 0){
                    $this->db->where($reqWhereArr)->limit(1)->update('request_billing_code', $reqBillData);
                    ...
                    $reqBillData['request_id'] = $record_id;
                    $reqBillData['specimen_id'] = $specimen_id;
                    $reqBillData['created_by'] = $this->ion_auth->user()->row()->id;
                    $this->db->insert('request_billing_code', $reqBillData);
                }
                */
			...
			//To update flag Comments 	
			$description = $this->input->post("flag_commentss");
			if($description!='')
			{
			$time_id = $this->input->post("record_id");
            $user_id = $this->ion_auth->user()->row()->id;
...
//            $retRes = $this->db->where(array('id'=>$time_id))->update("user_timesheet",$updateData);
            ...
            $whereArr = ['record_id' => $time_id, 'user_id' => $user_id, 'module_id' => $dataSection];
            if($this->db->get_where("section_comments", $whereArr)->num_rows() > 0){
                $retRes = $this->db->update("section_comments", ['description' => $description], $whereArr);
            }else{
...
//                $json['msg'] = '<div class="bg-success alert">Specimen Has Been Successfully Updated.</div>';
                $json['msg_txt'] = 'Specimen Has Been Successfully Updated.';
                echo json_encode($json);
                die;
            } else {
                $json['type'] = 'error';
                $json['msg'] = '<div class="bg-danger alert">Some how specimen did not updated successfully or updated already.</div>';
                $json['msg_txt'] = 'Some how specimen did not updated successfully or updated already.';
                echo json_encode($json);
                die;
            }
        }
    ...
    public function additional_work()
    {
        ...
        $additional_work = array('request_id' => $req_id, 'doctor_id' => $user_id, 'status' => 1, 'description' => $this->input->post('additional_description'), 'data_state' => 'in_session', 'additional_work_time' => date('Y-m-d h:i:s'));
        ...
        redirect('doctor/published_reports/2022/all', 'refresh');
    }
    ...
    public function publish_additional_work()
    {
        ...
        if (isset($_POST['request_id'])) 
		{
            ...
            $this->db->update('additional_work', array('data_state' => 'save_data', 'additional_work_time' => date('Y-m-d h:i:s')));
            $request_data = array(
    ...
    public function display_published_reports_ajax_processing()
    {
        ...
        foreach ($list as $record) 
		{
           ...
           // $dob_nd_nhs = '<br>' . $record->nhs_number;
		    $dob_nd_nhs = '';
            if (!empty($record->dob)) {
                $dob_nd_nhs =  date('d-m-Y', strtotime($record->dob));
            }
            $request_time = '';
...
//        endif;
	...
	{        
         ...
         if ($_GET['fw_page'] == 'requested' || $_GET['fw_page'] == 'complete') {
            //$fw_data1["requested"] = $this->Doctor_model->display_further_work_model_requested();
            /*$fw_data1["requested"] = $this->Doctor_model->display_further_work_model_requested_new();
            $this->load->view('doctor/inc/header-new');
            $this->load->view('doctor/fw_requested', $fw_data1);
            $this->load->view('doctor/inc/footer-new');*/
			
			
			 $fw_data1["requested"] = $this->Doctor_model->display_further_work_requested();
			 $this->load->view('doctor/inc/header-new');
			   $this->load->view('doctor/fw_requested_new', $fw_data1);
			    $this->load->view('doctor/inc/footer-new');
			
			
        } else if (!empty($_GET) && $_GET['fw_page'] == 'completed2') {
	...
	public function view_further_work_new()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
   ...
   public function display_edu_cases()
    {
        ...
        if (isset($_POST) && $_POST['edu_cats'] == 0) {
            ...
            $json_encode .= '<table id="doctor_record_list_table" class="table table-striped custom-table mb-0 dataTable no-footer" cellspacing="0" width="100%" style="margin-top:40px">';
            $json_encode .= '<thead><tr class="">';
            ...
            if (!empty($edu_cases)) {
                foreach ($edu_cases as $row) {
                    ...
                    $json_encode .= '</td>';
                    $json_encode .= '<td><a href="' . site_url() . '/doctor/doctor_record_detail_new/' . $row['uralensis_request_id'] . '"><img src="' . base_url('assets/img/detail.png') . '"></a></td>';
                    $json_encode .= '<td>';
...
//            $retRes = $this->db->where(array('id'=>$time_id))->update("user_timesheet",$updateData);
            ...
            if($retRes){
                ...
                $response['msg'] = "Comment Posted Successfully";
                $response['html'] = $getCommentDetails;
            } else {
                $response['type'] = "fail";
                $response['msg'] = "Failed to add detail. Please try again.";
            }
            echo json_encode($response);exit;
        }
    }
	
	public function save_hl7_comments()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
...
//            $retRes = $this->db->where(array('id'=>$time_id))->update("user_timesheet",$updateData);
            ...
            $whereArr = ['uralensis_request_id' => $time_id];
            $retRes = $this->db->update("request", ['hl7_content' => $this->input->post("hl7_comment")], $whereArr);
            $patient_id = $this->db->select('patient_id')->get_where("request", $whereArr)->row()->patient_id;

            $this->db->insert('generated_file_message_data', [
                'patient_id'        => $patient_id,
                'request_id'        => $time_id,
                'message_control_id'=> $this->input->post("hl7_MessageControleId"),
                'file_type'         => 'HL7',
                'file_name'         => $this->input->post("hl7_fileName"),
            ...
            if($retRes){
                $response['type'] = "success";
                $response['msg'] = "HL7 Content Added Successfully";
...
//            $this->db->update("opinion_comment_likes",$dataArray);
    ...
    public function set_input_change_color()
    {
       ...
       // $this->db->update('request_assignee', ['user_id' => $_POST['assigned_user_id']], ['request_id' => $record_id]);
		...
		$Rdata = array(
			...
			'request_type' => $_POST['specimen_nature'],
			'lab_id' => $_POST['lab_name'],
			'location' => $_POST['location'],
			'hospital_group_id' => $_POST['hos_id'],
			'dermatological_surgeon' => $_POST['dermatological_surgeon'],
			'date_taken' => $_POST['date_taken'],
			'date_received_bylab' => $_POST['date_received_bylab'],
			'date_sent_touralensis' => $_POST['date_sent_touralensis'],
			'pci_number' => $_POST['pci_number'],
			'speciman_no' => $_POST['specimen_no'],
			'record_batch_id' => $_POST['courier_no'],
            'medicare_card_no' => $_POST['medicare_card_no'],
		...
		$Pdata = array(
			...
			'hospital_id' => $_POST['hos_id']
        );
		$record_id=$_POST['patient_id'];
        $this->db->where('id', $record_id);
        $this->db->update('patients', $Pdata);
		
        $p_resultother=get_table_data("patient_other_details","patient_id=".$record_id);
        $other_data = array(
            'patient_id' => $record_id,
            'mrn_number' => $_POST['nhs_number'],
            'health_fund_code' => $_POST['health_fund_code'],
            'policy_number' => $_POST['policy_number'],
            'health_fund_name' => $_POST['health_fund_name'],
            'pensioner_card_number' => $_POST['pensioner_card_number'],
        ...
        if(count($p_resultother) > 0){
            $this->db
                ->where('patient_id', $record_id)
    ...
    public function search_and_add_barcode_record()
    {
        ...
        if (isset($_POST) && $_POST['search_type'] === 'only_search') {
            ...
            $request = array(
                ...
                'doctor_mdr_number' => time(), //hl7 final feedback
    ...
    public function add_specimen_doctor($specimen_request_id)
    {
		...
		$count_data=$this->input->post('count_speciman');
        ...
        $this->db->insert('request_specimen', $data);
		 ...
		 $specimens = $this->db->select('*')
                ->where('rs_request_id', $specimen_request_id)
                ->get('request_specimen')->result();
        $speciment_number = count($specimens);

        $defaultTestName = 'H&E L1';
        $insData[0] = [
        'specimen_id' => $specimen_id,
        'specimen_no' => $speciment_number,
        'block_no' => $speciment_number."A",
        'name' => $defaultTestName,
        'description' => $defaultTestName
        ];
        $this->Doctor_model->add_specimen_block($insData);
...
//        redirect('/doctor/doctor_record_detail/' . $specimen_request_id);
	...
	public function LoadFurtherWorkData(){
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        $where_search = '';
        if($this->input->post('filterStatus') != ''){
            $where_search = ' AND block.fw_status = "'.$this->input->post('filterStatus').'"';
        }
        $id = $this->input->post('fwId');
        ...
        WHERE fwd.further_work_id = $id ".$where_search." GROUP BY id ORDER BY id desc";
        $query = $this->db->query($sql);
        $data['specimen_blocks_listing']=$query->result();
        $data['labNumber']=$this->input->post('labNumber');
        $data['fwId']=$id;
        $data['postData'] = $this->input->post();
        echo json_encode($this->load->view('load_furtherwork_view', $data, true));
        exit;
    }
...
//                 Send email and attach uploaded images to email
    ...
    public function addSpecimenBlock()
    {
        ...
        if ($_POST['specimen_id']) 
		{
            ...
            //$this->form_validation->set_rules('test_ids[]', 'Test', 'required|trim');
            if ($this->form_validation->run() == FALSE) {
                $json["type"] = "fail";
                $json["msg"] = validation_errors();
                echo json_encode($json);
			...
			else 
			{
                ...
                $blockDescription = $this->input->post('test_ids');
                $testDescription = ($this->input->post('test_description') != '') ? explode(',',$this->input->post('test_description')) : '';
                $testName = ($this->input->post('test_name') != '') ? explode(',',$this->input->post('test_name')) : '';
               
                $totalLength = count($specimenNo);
                $defaultTestName = 'H&E L1';
                $insData[0] = [
                    'specimen_id' => $specimen_id,
                    'specimen_no' => $specimenNo[0],
                    'block_no'    => $blockName[0],
                    'name'        => $defaultTestName,
                    'description' => $defaultTestName
                ];
                $createHtml = "<tr><td>$lab_no</td><td>$blockName[0]</td><td>$defaultTestName</td></tr>";
                if(isset($blockDescription)){
                    for ($i = 1; $i <= count($blockDescription); $i++) {
                        $insData[$i]['specimen_id'] = $specimen_id;
                        $insData[$i]['specimen_no'] = $specimenNo[0];
                        $insData[$i]['block_no'] = $blockName[0];
                        $insData[$i]['description'] = $testDescription[$i-1];
                        $insData[$i]['name'] = $testName[$i-1];
    
                        $createHtml .= "<tr>";
                        $createHtml .= "<td>" . $specimenNo[$i] . "</td>";
                        $createHtml .= "<td>" . $blockName[$i] . "</td>";
                        $createHtml .= "<td>" . $testName[$i] . "</td>";
                        $createHtml .= "</tr>";
                    }
                }
...
//                $createHtml .= "</tr>";
    ...
    public function editSpecimenBlock()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        ...
        $json = array();
        $res = 0;
        if ($_POST['sid'] && count($this->input->post('test_ids')) > 0) 
		{
            $specimen_id = $this->input->post('sid');
            $specimenNo = $this->input->post('sno');
            $blockName = $this->input->post('bno');
            $blockDescription = $this->input->post('test_ids');
            $testDescription = $this->input->post('description');
            $testName = $this->input->post('name');
            
            $createHtml = "";
            $insData = array();
            $j = 0;
            for ($i = 0; $i < count($blockDescription); $i++) {
                $blockExist = $this->Doctor_model->check_specimen_block_list_exist($specimen_id,$specimenNo, $blockName,$testName[$i]);
                if($blockExist == 0 && $blockDescription[$i] != 0){
                    $insData[$j]['specimen_id'] = $specimen_id;
                    $insData[$j]['specimen_no'] = $specimenNo;
                    $insData[$j]['block_no'] = $blockName;
                    $insData[$j]['description'] = $testDescription[$i];
                    $insData[$j]['name'] = $testName[$i];

                    $createHtml .= "<tr>";
                    $createHtml .= "<td>" . $specimenNo . "</td>";
                    $createHtml .= "<td>" . $blockName . "</td>";
                    $createHtml .= "<td>" . $testName[$i] . "</td>";
                    $createHtml .= "</tr>";
                    $j++;
            ...
            if(count($insData) > 0){
                ...
                $tname = "'" . implode ( "', '", $testName ) . "'";
                $res = $this->Doctor_model->delete_specimen_block($specimen_id,$specimenNo, $blockName,$tname);
            }
    ...
    public function editBillingCode()
    {
        ...
        $json = array();
        $res = 0;
        if ($this->input->post(['request_id']) && count($this->input->post('billing_code_ids')) > 0) 
		{
            $requestId = $this->input->post('request_id');		
            $billingCodes = implode(",", $this->input->post('billing_code_ids'));		
            $change_billingCode = array('billing_code_ids' => $billingCodes);
            $this->db->where('uralensis_request_id', $requestId);
            $this->db->update('request', $change_billingCode);
            $json['type'] = 'success';
            $json['msg'] = "Billing updated successfully";
            echo json_encode($json);
            die;
        } else {
            $json['type'] = 'success';
...
//                     Check if doctor is assigned request
    ...
    public function add_further_work_new($id)
    {
        ...
        if (isset($id) && !empty($id)) {
            ...
            $labListArr = $this->Doctor_model->getLabNamesFromLabGroups();
            $recordDetailsArr = $this->Doctor_model->getDetailsAgainstRequest($recordId);
            if($recordDetailsArr && $recordDetailsArr[0]['lab_id'] > 0)
			{
                //Get all level 1
                $labId = $recordDetailsArr[0]['lab_id'];
                ...
                foreach ($data["testMainCategories"] as $mainCatArray => $mainCatValue)
				{
                    ...
                    $getSubCatAgainstMain = getSubTestCatAgainstMainCat($mainCatValue["id"]);
                    if(count($getSubCatAgainstMain) > 0){
                        $testSubCategoryArr["main_cat"][$i]["main_cat_name"] = $mainCatValue["name"];
                        $testSubCategoryArr["main_cat"][$i]["main_cat_id"] = $mainCatValue["id"];
                        $s = 0;
                        foreach ($getSubCatAgainstMain as $subValue => $subKey) {
                            ...
                            #Add tests under sub Categories
                            $t = 0;
                            $subCategoriesTests = getTestAgsinstSubCatNew($labId, $mainCatValue["id"], $subKey["id"]);
                            //pre($subCategoriesTests);
                            foreach ($subCategoriesTests as $testKey => $testValue) {
                                $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_id"] = $testValue["id"];
                                $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_name"] = $testValue["test_id"];
                                $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_desction"] = $testValue["name"];
                        ...
                        $t = 0;
                        $subCategoriesTests = getTestAgsinstSubCatNew($labId, $mainCatValue["id"]);
                        $testSubCategoryArr["main_cat"][$i]["main_cat_name"] = $mainCatValue["name"];
                        $testSubCategoryArr["main_cat"][$i]["main_cat_id"] = $mainCatValue["id"];
                        foreach ($subCategoriesTests as $testKey => $testValue) {
                            $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["tests"][$t]["test_id"] = $testValue["id"];
...
//pre($testSubCategoryArr);
                $data["test_sub_categories"] = $testSubCategoryArr;
            }



            $data["recordDetails"] = $recordDetailsArr;
            $data["labName"] = $labListArr;
...
//                     Check if doctor is assigned request
			...
			$sql = "SELECT block.* FROM specimen
            INNER JOIN specimen_blocks as block on block.specimen_id=specimen.specimen_id
            INNER JOIN further_work_detail as fwd on fwd.block_id=block.specimen_id AND fwd.test_id = block.specimen_no
            AND specimen.request_id = $id ORDER BY id desc";
        	$query = $this->db->query($sql);
            $data['specimen_blocks_listing']=$query->result();
			
            $data['specimen_blocks'] = $this->Doctor_model->specimen_block_detail($id);
            $data['block_no_list'] = $this->Doctor_model->specimen_block_list($id);
            $data['specimen_blocks_with_tests'] = $this->Doctor_model->specimen_block_detailwithTests($id);

            // print_r($data['specimen_query']); exit();
            $result = array_merge($data, $req_from_to_data, $reporting_doctors);
//            echo "<Pre>";
//            print_r($specimen_data['specimen_blocks']);
//            exit;
            //pre($data['test_sub_categories']);
            $data['primary_id'] = $id;
            $this->load->view('templates/header-new');
    ...
    public function delete_test_data($id, $testId){
        if(!empty($testId)){
            $whereArr = ['id' => $testId];
			$whereArr2 = ['request_id' => $id];
            $row = $this->db->get_where('specimen_blocks', $whereArr)->row();
            if(isset($row)){
                $this->db->delete('further_work_detail', ['block_id' => $row->specimen_id, 'test_id' => $row->specimen_no]);
                $this->db->delete('specimen_blocks', ['specimen_id' => $row->specimen_id, 'specimen_no' => $row->specimen_no]);
                //redirect('/doctor/add_further_work_new/'.$id);
            }
        }
        redirect('/doctor/add_further_work_new/'.$id);
    }

    public function getFurtherWorkTabView(){
        if(!empty($_POST['lab_id'])){
            $labId = $_POST['lab_id'];
            $testMainCategories = getMainTestCategories();
            $testSubCategoryArr = array();
            $i = 0;
            foreach ($testMainCategories as $mainCatValue) {
                $getSubCatAgainstMain = getSubTestCatAgainstMainCat($mainCatValue["id"]);
                $testSubCategoryArr["main_cat"][$i]["main_cat_name"] = $mainCatValue["name"];
                $testSubCategoryArr["main_cat"][$i]["main_cat_id"] = $mainCatValue["id"];
                $s = 0;
                foreach ($getSubCatAgainstMain as $subKey) {
                    //$testSubCategoryArr["main_cat"][$i][$mainCatValue["name"]]["sub_cat"][$s]["sub_cat_id"] = $subKey["id"];
                    $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["sub_cat_name"] = $subKey["name"];
                    $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["sub_cat_id"] = $subKey["id"];
                    $testSubCategoryArr["main_cat"][$i]["sub_cat"][$s]["main_cat_id"] = $subKey["main_category_id"];
            ...
            $data["test_sub_categories"] = $testSubCategoryArr;
            $result = $this->load->view('doctor/further_work_tab_view', $data, true);

            if($result) {
                echo json_encode(['status' => 'success', 'html' => $result]);exit;
            }else{
    ...
    public function save_further_request()
    {
        ...
        $work = array(
            ...
            'doctor_id' => $this->ion_auth->user()->row()->id,
            'fw_status' => 'requested',
			'request_by' => $this->ion_auth->user()->row()->id,
        ...
        for ($i = 0; $i < $arrayCount; $i++) 
		{
			...
			if($t_id!='')
			{
				$test_res = $this->db->query("SELECT * FROM laboratory_tests WHERE id = $t_id")->result_array();
                    if (count($test_res) != 0) 
					{
                        $test_n = $test_res[0]['test_id'];
						$test_desc = $test_res[0]['name'];
					}
				
				$data = array(
				'specimen_id'=>$blockIds[$i],
				'specimen_no'=>$t_id,
                'block_no'=>$block_no[$i],
				//'block_no'=>$this->random_generate_block_number(),
				'name'=>$test_n,
				'description'=>$test_desc,
				'created_by'=>$this->ion_auth->user()->row()->id	
				);				
				$this->db->insert('specimen_blocks',$data);
			}
        }
        $this->Doctor_model->add_further_work_detail($further_work_detail);

        $this->addLabEnquiry($_POST);
    ...
    public function addLabEnquiry($post){
        $labStatus= 0;
        $laboratoryId = ($labStatus == 1) ? $post['user_lab_id'] : $this->ion_auth->get_user_group_type()->row()->id;
        ...
        $ticketSaveArr = array(
            "lab_id"            => $laboratoryId,
            "ticket_type"       => 'GE',
            "ticket_subject"    => "Further Work Requested By $userData->enc_first_name $userData->enc_last_name",
            "ticket_message"    => 'Test: '. strtoupper(implode(', ', $post['test_name'])) .' & Reason for Request: '. $post['request_reason'],
            "ticket_priority"   => 'normal',
            "ticket_reference"  => $post['request_comment'],
            "user_type"         => $userType,
            "ticket_created_by" => $userData->id,
    ...
    public function getLabTestsAgainstSubCategory()
    {
        ...
        //    echo $subCatId; exit; 
        $getTestAgsinstSubCat = getTestAgsinstSubCat($subCatId, 103);
...
public function updatePatientsRecord()
	...
	public function delete_test($record_id,$rec=0)
    {
        $this->db->query("DELETE FROM specimen_blocks WHERE id = $record_id");
        $this->session->set_flashdata('success', TRUE);
        $this->session->set_flashdata('message', 'Test Deleted Successfully.');
        //$current_date_year = date('Y');
        //$this->db->query("DELETE FROM request_specimen WHERE rs_request_id = $record_id");
        //$this->session->set_flashdata('record_status',"Test Deleted Successfully");
        redirect('doctor/doctor_record_detail_old/'.$rec, 'refresh');
    }

    public function delete_multiple_test($record_id, $rec=0)
    {
        $test_ids = urldecode($record_id);
        $this->db->query("DELETE FROM specimen_blocks WHERE id IN($test_ids)");
        $this->session->set_flashdata('success', TRUE);
        $this->session->set_flashdata('message', 'Test Deleted Successfully.');
        //$current_date_year = date('Y');
        //$this->db->query("DELETE FROM request_specimen WHERE rs_request_id = $record_id");
        //$this->session->set_flashdata('record_status',"Test Deleted Successfully");
        if(isset($_GET['stab'])){
            redirect('doctor/doctor_record_detail_old/'.$rec."?stab=".$_GET['stab'], 'refresh');
        }else{
            redirect('doctor/doctor_record_detail_old/'.$rec, 'refresh');
        }
    ...
    public function autopsy_search_patient()
    {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        ...
        if (isset($_REQUEST['query'])) {
            $search_query = $_REQUEST['query'];
            $sql = "SELECT DISTINCT id,first_name,last_name,dob FROM patients WHERE CONCAT(first_name,' ', last_name) LIKE '%{$search_query}%' GROUP BY id ORDER BY first_name";
            $resArr = $this->db->query($sql)->result();
            $array = array();
            foreach ($resArr as $row) {
                $array[$row->id]['id'] = $row->id;
                $array[$row->id]['first_name'] = $row->first_name;
                $array[$row->id]['last_name'] = $row->last_name;
                $array[$row->id]['dob'] = (!empty($row->dob) && $row->dob != '0000-00-00') ? "($row->dob)" : "";
            }
            echo json_encode($array); //Return the JSON Array
        }
    }
    ...
    public function getPatientDetail(){
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $requestID = $this->input->post('request_id');
            // $patientArr = $this->db->get_where('patients', ['id'=>$patientID])->row();
            $patientArr = (object) $this->patient->get_patient_data($patientID);
            if($patientArr){
                $reqWhere = ['uralensis_request_id' => $requestID];
                $oldPdata = $this->db->get_where('request', $reqWhere)->row();
                if($oldPdata->patient_id != $patientID){
                    $cnt = $this->db->get_where('patients', ['id'=>$oldPdata->patient_id + 1])->num_rows();
                    //pre($this->db->last_query());
                    if($cnt == 0){
                        $this->db->delete('patients', ['id' => $oldPdata->patient_id]);
                    }
                    /*$pCont = $this->db->select('id')->where([
                        'first_name' => $patientArr->first_name,
                        'last_name' => $patientArr->last_name
                    ])->get('patients')->num_rows();*/
                    //pre($pCont);
                }
                $dob = date('Y-m-d', strtotime($patientArr->dob));
                $diff = date_diff(date_create($dob), date_create(date('Y-m-d')), TRUE);
                $age = $diff->format('%y');
                $reqData = array(
                    'f_name'     => $patientArr->first_name,
                    'sur_name'   => $patientArr->last_name,
                    'gender'     => $patientArr->gender,
                    'dob'        => $patientArr->dob,
                    'age'        => $age,
                    'nhs_number' => $patientArr->mrn_number,
                    'patient_id' => $patientID
                );
                $this->db->update('request', $reqData, $reqWhere);
                echo json_encode(['status' => 'success', 'data' => $patientArr]);exit;
            }
        }
        echo json_encode(['status' => 'error', 'message' => 'Something went wrong, Please Try Again.']);exit;
    }
    /**
     * Function to get weekly request pathelogist wise for hospital
     * return Json data
     */
    public function get_weekly_request(){  
        $user_id = intval($this->input->get('user_id'));    
        $fdate = date('Y-m-d', strtotime($this->input->get('fdate')));
        $tdate = date('Y-m-d', strtotime($this->input->get('tdate')));    
        $result = $this->Doctor_model->get_weekly_request($user_id, $fdate, $tdate);
        echo json_encode($result);exit;
      }

    public function LoadPatientDetails(){
        $result['status'] = "fail";
        if($this->input->post('pid') && $this->input->post('recordId')){
            $patientID = $this->input->post('pid');
            $recordID = $this->input->post('recordId');
            $patientArr['hospitals'] = $this->patient->fetch_hospitals();
            $patientArr['patient'] = $this->patient->get_patient_data($patientID);
            $patientArr['recordId'] = $this->input->post('recordId');
            $patientArr['profile_picture_path'] = $this->patient->get_profile_picture($this->input->post('pid'));
            // echo "<pre>";
            // print_r($patientArr);die;
            $modalView = $this->load->view('doctor/loadPatientDetails',$patientArr,true);
            $result['status'] = "success";
            $result['modalView'] = $modalView;
    ...
    public function get_bill_codes($clinic_id, $userId){
        $result = $this->billing_model->get_bill_code_data($clinic_id, $userId);
        $this->output
    ...
    public function get_bill_detail($bill_id){
        $result = $this->billing_model->get_bill_detail($bill_id);
        $this->output
            ->set_content_type('application/json')
            ->set_output(json_encode($result));
    }

    private function random_generate_block_number($preFix='') {
        ...
        $result = ((!empty($preFix)) ? "$preFix - " : "") . implode($pass) . $random; //turn the array into a string
        return $result;
    }
    ...
    {
        $result['status'] = "fail";
        if($this->input->post() && $this->input->post('record_id')){
            $change_status = array('request_code_status' => $this->input->post('r_status'));
            $this->db->where('uralensis_request_id', $this->input->post('record_id'));
            $this->db->update('request', $change_status);
            $result['status'] = "success";
        }
	...
	public function getRequestInfo()
    {
        ...
        if($this->input->post() && $this->input->post('record_id')){
            $doctor_id = $this->ion_auth->user()->row()->id;
            $result_res = (array) $this->Doctor_model->get_next_lab_record($doctor_id, $this->input->post('requeststatus'), $this->input->post('record_id'));
            if(count($result_res) > 0){
                $result['status'] = "success";
                $result['request_id'] = $result_res['uralensis_request_id'];
            }
            //$data["query"] = $this->Doctor_model->lab_record_list($doctor_id);
        }
        echo json_encode($result);
        exit;
    }
	
	 public function updateFurtherRequestStatus()
    {
        $result['status'] = "fail";
        if($this->input->post() && $this->input->post('changeId') && $this->input->post('statusChange') != ''){
            if($this->input->post('actionType') == 'single'){
                $change_status = array('fw_status' => $this->input->post('statusChange'));
                $this->db->where('id', $this->input->post('changeId'));
                $this->db->update('specimen_blocks', $change_status);
                $result['status'] = "success";
            }else{
                $id = $this->input->post('changeId');
                $sql = "SELECT GROUP_CONCAT(block.id) as speBlockIds FROM specimen
                INNER JOIN specimen_blocks as block on block.specimen_id=specimen.specimen_id
                INNER JOIN further_work_detail as fwd on fwd.block_id=block.specimen_id AND fwd.test_id = block.specimen_no
                AND fwd.further_work_id = $id ORDER BY block.id desc";
                $query = $this->db->query($sql);
                $speBlockIds=$query->row_array();
                if(count($speBlockIds) > 0){
                    $change_status = array('fw_status' => $this->input->post('statusChange'));
                    $this->db->where_in('id', explode(",", $speBlockIds['speBlockIds']));
                    $this->db->update('specimen_blocks', $change_status);
                    $result['status'] = "success";
                }
            }
