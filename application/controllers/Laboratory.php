...
Class Laboratory extends CI_Controller {
    ...
    public function index() 
	{
        ...
        $data["task_count"] = $this->Doctor_model->task_count();
        $data["today_request_count"] = $this->Doctor_model->today_request_count();
        $data["today_request"] = $this->Doctor_model->today_request();
    ...
    public function network(){
		...
		{
            // Upload files here
            $config['upload_path'] = 'uploads/HL7/FNQHPathology/HL7_In';
            $config['allowed_types'] = 'oru|ORU|hl7|HL7|ORM|ack|pdf|PDF|txt';
            ...
            if(!empty($fileData)){
                foreach($fileData as $key => $value){
                    ...
                    $this->db->insert('network_uploads', $file_data);
                }
            }
        }

        $data['res'] =  $this->db->get('network_uploads')->result_array();
        foreach( $data['res'] as $key => $value){
           
            $id = $value['user_id'];
            $query = $this->db->query("SELECT AES_DECRYPT(first_name, '" . DATA_KEY . "') AS First from users where id=".$id);
            $name = $query->row()->First;
            $data['res'][$key]['user_name'] = $name;
        }
        $data['input'] = scandir(FCPATH . 'uploads/HL7/FNQHPathology/HL7_In');
        $data['output'] = scandir(FCPATH . 'uploads/hl7_output');
        $data['imported'] = scandir(FCPATH . 'uploads/hl7_imported');

        $this->load->view('laboratory/inc/header-new');
        $this->load->view('laboratory/network', $data);
        $this->load->view('laboratory/inc/footer-new');
    }
    ...
    public function delete_user($id){
        if(!empty($id)){
			...
			$this->db->delete('users_groups', ['users_id' => $id]);
			$this->db->delete('users_groups', ['users_id' => $id]);
...
//echo "<pre>";print_r($query->result_array());exit;
        ...
        foreach ($query->result_array() as $key => $row) 
		{
			...
			$order = "<input value='$test_order' data-id=".$row['id']." class='order_lab' type='text' style=width:50px>";
            $data[] = array(
    ...
    public function update_status(){
        ...
        $data = array('test_order' => $test_order);
        $this->db->where('id', $id);
        $this->db->update('laboratory_tests', $data);
    }
...
//        $this->data['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
        ...
        $this->data['cgroup_id'] = base64_decode($cgroup_id);
        $this->data['hsa'] = $hsa;
...
//                    ->set_output(json_encode(array('status' => 'error', 'message' => 'Error Creating, hospital user try again later')));
    ...
    public function lab_template(){
        ...
        $data['categories'] = getMainTestCategories();
        $data['lab_templates'] = $this->lab->get_lab_templates($this->user_id);
    ...
    public function edit_lab($id){        
        ...
        if($data['lab']){
            if ($this->form_validation->run() == FALSE) {             
                ...
                $lab_data['lab_nata'] = $this->input->post('hospital_nata_num');
    ...
    public function get_billing_data(){
        ...
        $data = array();
        $res = $this->billing_model->billingData();
        $cnt = 0;
        ...
        foreach ($res as $row){
            ...
            $data['data'][] = array(
                ...
                'tissue_type'   => !empty($row["tissue_type"]) ? $row["tissue_type"] : '-',
                'price'         => $row["price"],
                'id'            => $row["id"],
            );
    ...
    public function delete_billing_by_request($id, $request_id)
	{
       // $message = 'Something went wrong, Please Try Again.';
        if(!empty($id))
		{
            $bRow = $this->db->get_where('request_billing_code', ['id'=>$id])->row();
            $bill_id = $bRow->bill_code;
            if(isset($bill_id))
			{
                $res1 = $this->db->delete('request_billing_code', ['request_id' => $bRow->request_id, 'bill_code' => $bill_id]);
                //$res2 = $this->db->delete('billing_data', ['id' => $bill_id]);
                if($res1 && $res2){
                    $this->session->set_flashdata('success', TRUE);
                    $message = 'Billing code delete successfully';
                }else{
                    $this->session->set_flashdata('error', TRUE);
                }
            }else{
    ...
    public function delete_billing_from_request()
    {
        ...
        if ($this->input->post()) {
            ...
            if(!empty($bill_id)) {
                $res = $this->db->delete('request_billing_code', ['request_id' => $request_id, 'bill_code' => $bill_id]);
                //$res1 = $this->db->delete('request_billing_code', ['request_id' => $bRow->request_id, 'bill_code' => $bill_id]);
                //$res2 = $this->db->delete('billing_data', ['id' => $bill_id]);
                if($res){
                    $response = ['status' => 'success', 'message' => 'Billing code delete successfully'];
                }
            }
            if(!empty($request_id)){
                $response['data'] = $this->db->select('bill_code, bill_code_text, bill_description, bill_price')->group_by('bill_code')->get_where('request_billing_code', ['request_id' => $request_id])->result_array();
            }
        }
...
//pre($resArr);
        if(count($resArr) == 0){
            ...
            return redirect('index.php/lab/lab_record_list', 'refresh');
        }
        ...
        foreach ($resArr as $k=>$res)
		{
            ...
            $filePath = Request_Form_Read . "/$k";
            $destinationFilePath = Request_Form_Success . "/$k";
            $errorFilePath = Request_Error . "/$k";

            $message_control_id = '';
            if(isset($res['Message Header']))
			{
                $message_control_id = $res['Message Header']['Message Control ID'];
                if(!empty($message_control_id))
				{
                   $isGenerated = $this->db->get_where('generated_file_message_data', ['message_control_id' => $message_control_id])->num_rows();
                  
				    if($isGenerated > 0)
					{
                        copy($filePath, $errorFilePath);
                        unlink($filePath);
                    }
                }
            }
            ...
            if(isset($res['Patient Identification']))
			{
                ...
                if($patientID){
                    $doctorID = '';
                    //$doctorID = $this->addPathologistOrClinicalData($res['Patient Visit'], 'D');
                    $clinicID = $this->addPathologistOrClinicalData($res['Patient Visit'], 'C');
                    $requestID = $this->addRequestData($res['Observation Request'], $patientID, $doctorID, $clinicID);
                    if($requestID){
                        $count++;
                        $this->addMessageControlData($patientID, $requestID, $message_control_id, $k);
                        $this->addObservationResult($requestID, $res['Observation Result'], $res['original_content']);
                        //$this->addAutoPublishRequestData($patientID, $requestID, $k);
                        // file move
    ...
    private function getHL7files()
	{
        ...
        foreach($files as $file){
            ...
            if($ext != 'ORM') { continue; }
            $file_name=$file;
			$isGene = $this->db->get_where('generated_file_message_data', ['file_name' => $file_name])->num_rows();
...
//            pre("<hr><hr><hr>", false);
    ...
    private function getPID($row){
        return [
            ...
            'Phone Number Home'                 => $this->convertContactInfoNew(trim($row[13])),
    ...
    private function getOBR($row){
        return [
            ...
            'Relevant Clinical Information'     => trim($row[13]),
            'Specimen Received DateTime'        => trim($row[14]),
    ...
    private function convertContactInfo($str){
        $resArr = explode('^', $str);
        return [
            'text'                  => trim($resArr[0]),
            'Telecommuniction Code' => trim($resArr[1]),
            'Telecommuniction Type' => trim($resArr[2]),
            'Email'                 => trim($resArr[3]),
            'Country Code'          => trim($resArr[4]),
            'Area City Code'        => trim($resArr[5]),
            'Phone Number'          => trim($resArr[6]),
            'Extension'             => trim($resArr[7])
        ];
    }
    private function convertContactInfoNew($str){
        $contactInfoList = explode('~', $str);
        foreach ($contactInfoList as $contactInfo){
            $resArr = explode('^', $contactInfo);
            $res[] = [
                'text'                  => trim($resArr[0]),
                'Telecommuniction Code' => trim($resArr[1]),
                'Telecommuniction Type' => trim($resArr[2]),
    ...
    private function addPatientData($pArr){
        if(!empty($pArr['Patient Name']['First Name'])){
            ...
            // Using MRN number
            /*$patientExistData = $this->db->get_where('patient_other_details', ['mrn_number' => setValue($pArr['Patient ID'])])->row();
            if($patientExistData){
                return $patientExistData->patient_id;
            }*/
            ...
            $pEmail = '';
            foreach ($pArr['Phone Number Home'] as $contactData){
                if(!empty($contactData['Email'])){
                    $pEmail = $contactData['Email'];break;
                }
            }
            ...
            $patientArr = [
                ...
                'phone'         => setValue($pArr['Phone Number Home'][0]['Phone Number']),
                'town'          => setValue($pArr['Phone Number Home'][0]['Area City Code']),
                'email'         => setValue($pEmail),
				...
				'nhs_number'    => setValue($pArr['Patient Identifier List'][1]['ID']),
                'medicare_card_no' => setValue($pArr['Patient Identifier List'][1]['ID'])."/".setValue($pArr['Patient Identifier List'][1]['Check Digit'])
                
            ];

            $rowQuery = $this->db->get_where('patients', $checkPatientArr);
    ...
    private function addRequestData($reqArr, $pid, $doctor_id='', $clinic_id=''){

        $template_id = 14;
        $hospital_group_id = 127;
        $lab_id=128; // for AUS server
        //$lab_id=118; // for local server
        $this->load->model('DepartmentModel', 'department');

        $row = $tem_query = $this->db->query("SELECT * FROM uralensis_record_track_template where ura_rec_temp_id='$template_id'")->row();
        ...
        $dermatological_surgeon = (!empty($clinic_id)) ? $clinic_id : $row->temp_clinic_user;
...
//        $pic_no = 'PB22'.rand(1000,99999);
        ...
        $request = array(
            ...
            'speciman_no'                   => '1',
            'f_name'                        => $patient['first_name'],
			'clinical_history'              => $reqArr['Relevant Clinical Information'],
            'sur_name'                      => $patient['last_name'],
            'dob'                           => $patient['dob'],
        ...
        $specimen = array(
            ...
            'specimen_block'    => '1',
            'specimen_slides'   => '1',
			'specimen_clinical_history' => $reqArr['Relevant Clinical Information'],
            'numberOfSlides'    => '1',
            'numberOfBlocks'    => '1',
            'blockDetail'       => '1A',
            'slides'            => '0',
    ...
    private function addPathologistOrClinicalData($arr, $roleType){

        $attDr = $arr['Attending Doctor'];
        $refDr = $arr['Referring Doctor'];
        $conDr = $arr['Consulting Doctor'];
        $hospital_or_clinic_id = 127;
        ...
        $user_id = $this->ion_auth->check_existing_user($refDr['First Name'], $refDr['Last Name']);
        if($user_id > 0){
            return $user_id;
        }
        ...
        $additional_data = [
            ...
            'last_name'             => $this->db->escape($refDr['Last Name']),
			'provider_no'           => $this->db->escape($refDr['Number']),
            'company'               => $this->db->escape($refDr['Assigning Authority']),
            'phone'                 => $this->db->escape(NULL),
            'memorable'             => $this->db->escape('helloworld'),
            'is_hospital_admin'     => $is_hospital_admin,
            'profile_picture_path'  => $this->db->escape($profile_picture),
           // 'user_type'             => $this->db->escape($roleType), // D for doctor & C for clinic
		    'user_type'             => $this->db->escape($roleType), // D for doctor & C for clinic
            'sub_role'              => '',
    ...
    private function addObservationResult($request_id, $obResArr, $original_content){
        foreach ($obResArr as $obRes){
            $this->db->update('request', ['hl7_content' => $original_content], ['uralensis_request_id' => $request_id]);
            //$valType = setValue($obRes['Value Type']);
            //if(strtoupper($valType) == 'CE'){
                $observationArr = [
                    'request_id'    => $request_id,
                    'set_id'        => $obRes['Set ID'],
                    'value_type'    => setValue($obRes['Value Type']),
                    'ob_sub_id'     => setValue($obRes['Observation Sub-ID']),
                    'ob_value'      => setValue($obRes['Observation Value']),
                    'full_row'      => setValue(json_encode($obRes)),
                    'ob_date'       => dateCovertString($obRes['DateTime of the Observation']),
                ];
                $this->db->insert('observation_data', $observationArr);
            //}
        }
    }
    private function addMessageControlData($patient_id, $request_id, $message_control_id, $fileName){
    ...
    private function addAutoPublishRequestData($patient_id, $request_id, $fileName){
        ...
        $this->db->update('further_work', ['fw_status' => 'complete'], ['request_id' => $request_id]);

        $this->db->insert('uralensis_record_history', [
        ...
        $filePath = './' . Request_Form_Success . "/$fileName";
        $this->db->update('patients', ['hl7' => $filePath], ['id' => $patient_id]);
