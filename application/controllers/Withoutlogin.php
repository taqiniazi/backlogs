...
class Withoutlogin extends CI_Controller
{
    ...
    function __construct()
    {
		...
		$this->load->helper(array('url', 'cookie', 'ec_helper', 'globalfunctions_helper'));
    ...
    public function GenerateTokenByEmailId($email_id, $name)
    {
        ...
        if (in_array($_SERVER['REMOTE_ADDR'], $whitelist)) {
            ...
            $this->email->from('info@pathhub.fnqhpathology.com'); // change it to yours
            $this->email->reply_to('info@pathhub.fnqhpathology.com', 'Pathhub Support Team');
            $this->email->cc('appdev@oxbridgemedica.com');
            $this->email->to($email_id); // change it to yours
            $this->email->subject('FNQH Pathology Authentication Access Token');
            // $mesg = $this->load->view('auth/pinemailtemplate',$dataTemp,true);
            $this->email->message($message);

            //$this->email->message($mesg);
           // if ($this->email->send()) {
 			if(EmailSend($email_id, 'FNQH APP Authentication Access Token', $message)){
                $set_auth_cookie = array(
...
//                $this->Ion_auth_model->set_session($user, 'yes');
                if (isset($_SESSION['temp_data'])) {
                    ...
                    if (!isset($check_cookie)) {
                        ...
                        $this->email->message($message);
                        // if ($this->email->send()) {
                        if(EmailSend($email_id, 'Uralensis Authentication Access Token', $message)){
