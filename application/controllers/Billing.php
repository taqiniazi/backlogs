...
class Billing extends CI_Controller
{
    ...
    public function invoice()
    {
        ...
        $data = [];
        $data['invoice_data'] = $this->billing_model->invoiceData();
    ...
    public function bill_track(){
        ...
        $data['is_invoice_generate'] = (in_array($groupType, ['H', 'HA'])) ? true : false;
    ...
    public function preview_pdf(){
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $invoiceNumber = time();
            $data = $this->get_invoice_data($clinicId, $invoiceNumber);
...
//        $this->load->view('billing/generate-invoice', $data);
    ...
    public function get_invoice_data($clinicId, $invoiceNumber){
        $userId = $this->session->userdata('user_id');
        $groupId = $this->ion_auth->get_users_main_groups()->row()->id;

        $result = $this->billing_model->bill_track_invoice_data($userId, $groupId, $clinicId);
