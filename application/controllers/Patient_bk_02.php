...
//$pData->pdf_content=$this->db->get_where('request', ['uralensis_request_id' => $request_id])->row()->hl7_content;
					...
					$pData->rDate=$this->db->get_where('request', ['uralensis_request_id' => $request_id])->row()->date_received_bylab;
					$LabNumber=$this->db->get_where('request', ['uralensis_request_id' => $request_id])->row()->serial_number;
					$pData->pDate=$this->db->get_where('request', ['uralensis_request_id' => $request_id])->row()->publish_datetime;

					$pData->PHL7_Details=$this->db->get_where('hl7_details', ['lab' => $LabNumber])->row();	
...
//$res .= rtrim($res_val[2])."\n";
       ...
       // $case2 .= "OBX|1|FT|$reportType^^AUSLAB||$pdfData||||||F|||$pdate_final|^^AUSLAB \n";
...
//$hl7_content
    ...
    private function text_file_content_type3($data)
	{
       ...
       /* Patient Data */
		...
		//print_r($data->PHL7_Details->lab);
		
		$data->PHL7_Details->lab;
		$PID=$data->PHL7_Details->pid;
		$PID1=$data->PHL7_Details->pid1;
		$PID2=$data->PHL7_Details->pid2;
		$referDoctorId=$data->PHL7_Details->obr;
		$referDoctorFirstName=$data->PHL7_Details->obr1;
		$referDoctorLastName=$data->PHL7_Details->obr2;
		$const_Doctor_Assigning_Authority=$data->PHL7_Details->obr3;
		$referDoctorId2=$data->PHL7_Details->obr4;
		$referDoctorFirstName2=$data->PHL7_Details->obr5;
		$referDoctorLastName2=$data->PHL7_Details->obr6;
		$const_Doctor_Assigning_Authority2=$data->PHL7_Details->obr7;
		...
		else
		{
			...
			$res .="PID|1||$PID^^^$PID1^$PID2~$patientSSN^AUSMC^AUSHIC^NI||$pLastName^$pFirstName^^^||$pDob|$pGender|||^^$address2\r\n";
			
            //$res .= "PID|1|||^^^BPS^PI^$patientSSN^AUSMC^AUSHIC^MC|$pLastName^$pFirstName^^^||$pDob|$pGender|||$address\r\n";
           
		   $res .="PV1|1|O||||||$referDoctorId^$referDoctorFirstName^$referDoctorLastName^^^Dr^^^$const_Doctor_Assigning_Authority|$referDoctorId2^$referDoctorFirstName2^$referDoctorLastName2^^^Dr^^^$const_Doctor_Assigning_Authority2|||||||||||BULKBILL\r\n";
