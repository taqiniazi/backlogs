...
class Document_List extends CI_Controller
{
	...
	function monthly_task_checklist_view()
	{
		...
		$data['data'] = $this->db->select('log_data')->where('year', $year)->get('monthly_task')->row_array();
	...
	public function update_temprature_logs_weekly()
	{
		...
		echo json_encode(array('message' => 'Timesheet Log has been updated successfully!!!!'));
		exit;
	}
	...
	public function get_temprature_logs_weekly(){
		...
		$user_id = $this->ion_auth->user()->row()->id;
		$data['result'] = $this->db->where('user_id', $user_id)->where('week_date', $weekDate)->get('temperature_logs')->row_array();
	...
	public function update_weekly_timesheetData()
	{
		...
		foreach ($this->input->post() as $key => $post) {
			$date = date('Y-m-d', strtotime($key));
				$check_booked_out_status = $this->db->where('log_date', $date)->where('user_id', $user_id)->get('timesheet_logs')->row_array();
			if ($check_booked_out_status) {
	...
	public function update_monthly_task_checklist()
	{
		...
		foreach ($this->input->post() as $key => $post) {
			...
			$check_booked_out_status = $this->db->where('year', $date)->get('monthly_task')->row_array();
			if ($check_booked_out_status) {
				$timesheetLogs = array('log_data' => json_encode($this->input->post()));
				$this->db->where('id', $check_booked_out_status['id']);
				$this->db->where('user_id', $user_id);
