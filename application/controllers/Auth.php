...
class Auth extends CI_Controller {
    ...
    public function index() {        
        ...
        $labUserGroupArray = LAB_GROUP;
        $hospitalUserGroupArray = HOSPITAL_GROUP;
        $PathGroupArray = PATH_GROUP;
...
//                echo '<pre>'.$value->user_id; print_r($user_hosp);
        ...
        //    echo '<pre>'; print_r($this->data['usersList']); exit;
            ...
            $this->data["firstRowCounts"] = $this->Admin_model->getDashboardFirstRowCount();
            $this->data["hospital_networks"] = $this->Admin_model->getHospitalNetworks();
        ...
        else if (!empty($groups) && in_array($groups[0]->group_type, $hospitalUserGroupArray)) 
        {
            track_user_activity();
            redirect('institute/', 'refresh');
        } 
        else if (!empty($groups) &&  in_array($groups[0]->group_type, $PathGroupArray)) 
        {
            track_user_activity();
        ...
        else if (!empty($groups) && in_array($groups[0]->group_type, $labUserGroupArray)) 
        {
            track_user_activity();
...
//print "<br>";
//print_r($hospitalUserGroupArray);
            echo 'Please Contact To Administrator For Further Instructions.';
        }
    }

    public function dashoardDetails() {
        if (!$this->ion_auth->logged_in()) {
            //redirect them to the login page
            return redirect('auth/login', 'refresh');
        }
...
//            print_r($this->data['users']); exit; 
            ...
            $this->data["name"] = $this->input->post('name');
            $this->data["group_id"] = $group_id;
            $this->data['usersList'] = $this->Userextramodel->getAllusersForadminList($group_id, $this->input->post('name'), $this->input->post('status'));
            // echo '<pre>'; print_r($this->data['usersList']); exit;
            foreach ($this->data['usersList'] as $key => $value) {
                $user_hosp = $this->Userextramodel->get_user_hospitals_ids($value->user_id); //10
                $this->data['usersList'][$key]->user_hospitals = $user_hosp;
...
//            echo '<pre>'; print_r($this->data['usersList']); exit;
            ...
            //   print_r( $this->data['usersList']); exit; 

            track_user_activity();
            $f_data['javascripts'] = array('js/auth/index_details.js');
            $this->_render_page('templates/header-new');
            $this->_render_page('auth/index-new-details', $this->data);
            $this->_render_page('templates/footer-new', $f_data);
            ...
            foreach ($meta_data_user_detail as $key => $value) {
                ...
                if (!empty($db_meta)) {
                    $this->db->where('user_id', $db_meta['user_id'])
                            ...
                            ->update(
                                    'usermeta', array('meta_value' => $value)
                    );
                } else {
                    if (!empty($value)) {
...
public function delete_group($user_id) {
        ...
        $this->ion_auth->deleteGroup($user_id);
       // header('Content-Type: application/json');
        //echo json_encode(array('status' => 'success'));
        
        $this->db->delete('laboratory_information', ['group_id' => $user_id]);
        $this->db->delete('hospital_information', ['group_id' => $user_id]);
        redirect('/', 'refresh');     
    }
    ...
    public function create_user() 
    {
        ...
        if ($this->form_validation->run() === TRUE) 
        {
            ...
            $additional_data = [
                ...
                'provider_no' => $this->input->post('provider_no'),
                'given_name' => $this->input->post('given_name'),
                'title' => $this->input->post('title'),
                'address' => $this->input->post('address'),
                'sub_town' => $this->input->post('sub_town'),
                'state' => $this->input->post('state'),
                'pincode' => $this->input->post('postcode'),
                'practice_code' => $this->input->post('practice_code'),
                'online_claim_type' => $this->input->post('online_claim_type'),
                'payee_provider_no' => $this->input->post('payee_provider_no'),
                'ihih_number' => $this->input->post('ihih_number'),
                'notes' => $this->input->post('notes'),
                'enter_by' => $this->input->post('enter_by'),
                'updated_by' => $this->input->post('updated_by'),
                'enter_date' => $this->input->post('enter_date'),
                'update_date' => $this->input->post('update_date'),
                'active' => $this->input->post('in_active')
...
//        }
    ...
    public function sendUserLoginInformation($email, $password) {
        //        $config = array(
        //            'protocol' => 'sendmail',
        //            'mailtype' => 'html',
        //            'charset' => 'utf-8',
        //            'wordwrap' => TRUE
        //        );
        $res['details'] = $this->db->select("id,memorable as , AES_DECRYPT(email, '" . DATA_KEY . "') AS email, 
        memorable AS memorable")
                        ->where("AES_DECRYPT(email, '" . DATA_KEY . "') = ", $email)
                        ->get('users')->row();
                $res['details']->password = $password;
                $message = $this->load->view("auth/email/user_details", $res, TRUE);
                $config = array(
                    'mailtype' => 'html',
                    'charset' => 'utf-8', //iso-8859-1
                    'newline' => '\r\n',
        ...
        //        $cid = $this->email->attachment_cid($logo);
                ...
                $this->email->to($email);
        //        $this->email->set_header('Content-Type', 'text/html');
                $this->email->subject("Pathology Healthcare Account Credentials");
                $this->email->message($message);
                $this->email->send();
        //        if(!$this->email->send()){
        //            print_r($this->email->print_debugger());
    ...
    public function updateUserStatus() {
        ...
        $status = $this->input->post("dataId");
        $check = $this->db
                ->where(array("id" => $user_id))
                ->update("users", array("status" => $status));
        if ($check) {
            $response['status'] = "success";
            $response['message'] = "Status updated successfully";
        } else {
            $response['status'] = "fail";
            $response['message'] = "Status not updated. Please try again.";
        }
    ...
    public function verifyEmail($email) {
        ...
        if ($res != 0) {
            $_SESSION['email_activation'] = "Email Activated Successfully";
        }
        redirect("auth", 'refresh');
    }

    public function verifyAdminEmail($email, $password) {
        $email = base64_decode($email);
        ...
        if ($res != 0) {
            $_SESSION['email_activation'] = "Email Activated Successfully";
            $this->sendUserLoginInformation($email, $password);
        }
        redirect("auth", 'refresh');
    }

    public function verifyActivity($userData) {
        $userData = base64_decode($userData);
        $userData = explode("___", $userData);
        $this->db
                ->where("session_userid", $userData[0])
                ->where("client_ip", $userData[1])
                ->where("remember", 0)
                ->update('userlogin_activity', array('remember' => $userData[2]));
        $_SESSION['email_activation'] = "Your account has been verified. Proceed to Login.";
        if ($userData[2] == 2) {
            $this->data['password_change'] = TRUE;
            $this->data['user_id'] = $userData[0];
            $includes['styles'] = array('password/css/jquery.passwordRequirements.css');
...
//            $this->_render_page('templates/footer-new', $includes);
    ...
    public function create_hospital() 
    {
        ...
        else if ($this->input->method() === 'post') 
        {
            ...
            if ($user_ids) 
            {
                ...
                $temp_data = array(                   
                    'institute_id' => $this->input->post('Hgroup_id')                   
                );
                
                $this->db->where('user_id', $user_ids);
                $this->db->update('users_groups', $temp_data);
                ...
                {
                return redirect('auth/create_hospital', 'refresh');
                    $this->output
    ...
    public function edit_user($id) 
    {
        ...
        $uDetails = get_usergroup_tyep_cate($id);
        $user_hospital_list = get_related_hospital($uDetails['user_id'],$uDetails['groupid'],$uDetails['group_type'],$uDetails['type_cate']);
...
//            exit;
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if($this->input->post('login_token') != ''){
                    $getpinsCheck = getRecords("*", "tbl_pins", array("useremail" => $decryptedDetails->email));
                    if(count($getpinsCheck) > 0){
                        $data = array(
                            'token' => $this->input->post('login_token'),
                        );
                        $this->db->where(array("useremail"=>$decryptedDetails->email))->update('tbl_pins', $data);
                    }else{
                        $data = array(
                            'useremail' => $decryptedDetails->email,
                            'token' => $this->input->post('login_token'),
                            'token_status' => 1
                        );
...
//                            $this->input->post('group_id'), $this->input->post('memorable'), $this->input->post('login_token'), $user->id, $this->input->post('first_name'), $this->input->post('last_name'), $this->input->post('company'), $this->input->post('email'), $this->input->post('phone'), $image_path, !empty($profile_image), !empty($this->input->post('hospital_admin')), ($current_email != $pre_email) ? 0 : $this->input->post("pre_status")
//                    );
//                } else {
//                    //check to see if we are updating the user
//                    if ($this->ion_auth->update($user->id, $data)) {
...
//                }
            ...
            if (!empty($db_meta)) {
                $this->db->where('user_id', $db_meta['user_id'])
                            ->where('meta_key', $db_meta['meta_key'])
                            ->update('usermeta', array('meta_value' => $value));                
            }
            else {
             if (!empty($value)) {
                        $this->db->insert('usermeta', array('user_id' => $id, 'meta_key' => $key, 'meta_value' => $value));
...
//                }
//            }
            // Code for saving user certificates
             $formData = $this->input->post();
             if ($formData['action'] == "update_certificate") {

                $certificate_title = $formData['certificate_title'];
                $certificate_date = $formData['certificate_date'];
                $certificate_city = $formData['certificate_city'];
                $certificate_country = $formData['certificate_country'];
                $online_personally = $formData['online_personally'];
                $admin_approved = $formData['admin_approved'];
                $duration_course_to = $formData['duration_course_to'];
                $duration_course_from = $formData['duration_course_from'];
                $institution_name = $formData['institution_name'];

                // print_r($certificate_city); die;
                // Prepare data
                 $data = array(
                        ...
                        'online_personally'=>$online_personally,
                        'admin_approved'=>$admin_approved == 'on' ? 1 : 0,
                        'duration_course_to'=>$duration_course_to,
                        'duration_course_from'=>$duration_course_from,
                        'user_id' => $id
                    );
                // Check if records already exist
                $query = $this->db->get_where('user_certificates', array('user_id' => $id));
                if ($query->num_rows() == 0) 
                    $this->db->insert('user_certificates',$data);
                else 
                    $this->db->update('user_certificates',$data,'user_id='.$id);
        ...
        $this->data['user_details'] = array(
            ...
            'provider_no'=>$this->form_validation->set_value('provider_no', $user->provider_no),
            'dob'=>$this->getUserMetaData($id, 'dob'),
            'street_address'=>$this->getUserMetaData($id, 'street_address'),
            'city'=>$this->getUserMetaData($id, 'city'),
            'state'=>$this->getUserMetaData($id, 'state'),
            'street_address'=>$this->getUserMetaData($id, 'street_address'),
...
//        print_r($member_of_group_id); exit;
        $this->data['user_group_id'] = $group_id;
        $this->data['child_groups'] = $this->Admin_model->getChildGroupsByUserId($user->id);
...
//            echo '<pre>'; print_r($this->data['child_pathologist_groups']); exit;
        ...
        $user_member_groupsInfo = $this->Admin_model->getParentGroupsDetailsByUserId($user->id);
        if(!empty($get_institute_ids)){
            $member_of_group_id = $get_institute_ids;
        }
        $this->data['user_member_groups'] = $member_of_group_id;
        $this->data['user_member_groupsInfo'] = $user_member_groupsInfo;
//        echo '<pre>'; print_r($this->data['user_child_group']); exit;
...
//     exit;
        ...
        $uDetails = get_usergroup_tyep_cate($id);
        $this->data['linked'] = get_related_hospital($uDetails['user_id'],$uDetails['groupid'],$uDetails['group_type'],$uDetails['type_cate']);
...
//        echo "<pre>";print_r($this->data['usersLeaveBalance']);exit;
    ...
    public function create_group() {
        ...
        if ($this->form_validation->run() == TRUE) {
            $new_group_id = $this->ion_auth->create_group_main(
                    strtolower(str_replace(' ', '', $this->input->post('group_name'))), $this->input->post('first_initial'), $this->input->post('last_initial'), $this->input->post('description'), $this->input->post('group_type'), $this->input->post('group_info'), $this->input->post('group_lab_number_format')
            );
            if ($new_group_id) {
                // check to see if we are creating the group
                // redirect them back to the admin page
    ...
    if($id!='')
    {
        ...
        if($group->group_type==H)
        {
            ...
            $this->db->update('hospital_information', $info_data);
        }

        if($group->group_type==L)
        {
            $info_data = array(                        
                            'lab_address' => $this->input->post('hospital_address'),                        
                            'lab_city' => $this->input->post('hospital_city'),
                            'lab_state' => $this->input->post('hospital_state'),
                            'lab_post_code' => $this->input->post('hospital_post_code'),
                            'lab_phone' => $this->input->post('hospital_number')
                        );
            
            $this->db->where('group_id', $id);
            $this->db->update('laboratory_information', $info_data);
        }
        ...
        if($group->group_type==L)
        {
            $query = $this->db->get_where('laboratory_information', array('group_id' => $group->id));
            foreach ($query->result() as $row)
            {
                   // echo $row->title;
            }       
       $this->data['hospital_address'] = array(
            'name' => 'hospital_address',
            'id' => 'hospital_address',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_address', $row->lab_address),
        );
        $this->data['hospital_city'] = array(
            'name' => 'hospital_city',
            'id' => 'hospital_city',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_city', $row->lab_city),
        );
       $this->data['hospital_state'] = array(
            'name' => 'hospital_state',
            'id' => 'hospital_state',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_state', $row->lab_state),
        );
        $this->data['hospital_post_code'] = array(
            'name' => 'hospital_post_code',
            'id' => 'hospital_post_code',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_post_code', $row->lab_post_code),
        );
        $this->data['hospital_number'] = array(
            'name' => 'hospital_number',
            'id' => 'hospital_number',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_number', $row->lab_phone),
        );
        ...
        if($group->group_type==H)
        {
            $query = $this->db->get_where('hospital_information', array('group_id' => $group->id));
            foreach ($query->result() as $row)
            {
                   // echo $row->title;
            }       
       $this->data['hospital_address'] = array(
            ...
            'id' => 'hospital_address',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_address', $row->hosp_address),
        );
        $this->data['hospital_city'] = array(
            'name' => 'hospital_city',
            'id' => 'hospital_city',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_city', $row->hosp_city),
        );
       $this->data['hospital_state'] = array(
            'name' => 'hospital_state',
            'id' => 'hospital_state',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_state', $row->hosp_state),
        );
        $this->data['hospital_post_code'] = array(
            'name' => 'hospital_post_code',
            'id' => 'hospital_post_code',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_post_code', $row->hosp_post_code),
        );
        $this->data['hospital_number'] = array(
            'name' => 'hospital_number',
            'id' => 'hospital_number',
            'type' => 'text',
            'value' => $this->form_validation->set_value('hospital_number', $row->hosp_phone),
        );
    ...
    public function setting_validation_is_unique_hospital_name() 
    {
        if (!$this->ion_auth->logged_in()) 
        {
            $this->output->set_status_header(401);
       ...
       $name = $this->input->get('name');
        ...
        $row = $this->db->query("SELECT * FROM `groups` WHERE id!='$hospital_id' and description='$name'")->num_rows(); 
       // $row = $this->db->get_where('groups', array('description' => $name, 'id'!=>$hospital_id))->num_rows();
        $this->output
                ->set_content_type('application/json');
    ...
    public function validation_is_unique_user_name() {
        if (!$this->ion_auth->logged_in()) {
            $this->output->set_status_header(401);
            echo "Not authorized";
            return;
        ...
        if (empty($name)) {
            $this->output->set_status_header(400);
            echo "Provide clinic name";
            return;
        }

        $row = $this->db->query("SELECT * FROM `groups` WHERE `groups`.`name`  = '$name' AND `groups`.`group_type` = 'H'")->num_rows();
        $this->output->set_content_type('application/json');
    ...
    public function get_all_roles1() {
        if (!$this->ion_auth->logged_in()) {
            $this->output->set_status_header(401);
            echo "Not authorized";
            return;
        ...
        $this->db->order_by('description', 'ASC');
        $res = $this->db->get_where('groups', array('type_cate' => 'category','parent_group_type'=>'H'))->result_array();
        $retData['H'] = $res;
        $this->db->order_by('description', 'ASC');
        $res = $this->db->get_where('groups', array('type_cate' => 'category','parent_group_type'=>'L'))->result_array();
        $retData['L'] = $res;
        $this->db->order_by('description', 'ASC');
        $res = $this->db->get_where('groups', array('type_cate' => 'category','parent_group_type'=>'D'))->result_array();
        $retData['D'] = $res;
        $this->db->order_by('description', 'ASC');
        $res = $this->db->get_where('groups', array('type_cate' => 'category','parent_group_type'=>'S'))->result_array();
        $retData['S'] = $res;
        $this->db->order_by('description', 'ASC');
        $res = $this->db->get_where('groups', array('type_cate' => 'category','parent_group_type'=>'A'))->result_array();
        $retData['A'] = $res;
        //echo $this->db->last_query(); exit;
        $this->output
                ->set_content_type('application/json')
    ...
    public function update_password() {
        ...
        if (!$this->ion_auth->logged_in() and $pass_check != 1) {
            redirect('auth/login', 'refresh');
        }
        ...
        if ($this->input->post("pass_status")) {
            ...
            if (!$this->ion_auth->is_admin()) {
                $this->form_validation->set_rules('prepass', 'Current Password', 'trim|required|min_length[3]');
            }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->ion_auth->verify_password($prepass, $user->password) or ( $this->ion_auth->is_admin())) {
    ...
    public function update_pin() 
    {
        ...
        $user_pin = implode("", $user_pin);
        $_POST['user_pin_c'] = $user_pin;
        $this->form_validation->set_rules('user_pin_c', 'PIN', 'required');
        if ($this->form_validation->run() == FALSE) 
        {
            $response['status'] = "fail";
            $response['message'] = validation_errors();
        ...
        {
            $checkStatus = $this->db->where('id', $userId)->update("users", array("user_auth_pass" => $user_pin));
            if ($checkStatus) 
            {
                $response['status'] = "success";
                $response['message'] = "Pin Code updated successfully";
            } 
            else 
            {
                $response['status'] = "fail";
                $response['message'] = "There might be some error. Please try again";
            }
        }
        echo json_encode($response);
        exit;
    ...
    public function get_child_groups(){
        $data['status'] = 'error';
        $data['message'] = 'There is some problem';
        if($this->input->post()){
            $group_type = $this->input->post('group_type');
            $data['parent_groups'] = $this->Admin_model->getChildGroupsByType($group_type);
            $data['status'] = 'success';
    ...
    public function get_role_hospitals(){
        $data['status'] = 'error';
        $data['message'] = 'There is some problem';
        if($this->input->post()){
            $group_type = $this->input->post('group_type');
            $groupType = $this->db->where(array("id"=>$group_type))->select("parent_group_type")->get("groups")->row()->parent_group_type;
            if($groupType=="H" || $groupType=="L"){
                $allGroups = $this->db->where("group_type IN ('L','H','D')")->select("*")->get("groups")->result_array();
            }
            else if($groupType=="D"){
                $allGroups = $this->db->where("group_type IN ('L','H','D')")->select("*")->get("groups")->result_array();
            } else {
                $allGroups = $this->db->where(array("group_type"=>$groupType))->select("*")->get("groups")->result_array();
            }

            $data['parent_groups'] = $allGroups;
            $data['status'] = 'success';
            $data['message'] = 'Data Found!';
        }
        echo json_encode($data);
    ...
    public function get_user_sub_role(){
        $data['status'] = 'error';
        $data['message'] = 'There is some problem';
        if($this->input->post()){
            $group_type = $this->input->post('group_type');
            $allGroups = $this->db->where(array("parent_id"=>$group_type))->select("*")->get("groups")->result_array();
            $data['parent_groups'] = $allGroups;
            $data['status'] = 'success';
            $data['message'] = 'Data Found!';
        }
    ...
    public function get_all_pathologists(){
        $data['status'] = 'error';
        $data['message'] = 'There is some problem';
        if($this->input->post()){
            $data['pathologists'] = $this->Admin_model->getAllPathologists();
            $data['status'] = 'success';
        }
        echo json_encode($data);
        exit;
...
//        exit;
        ...
        else
        {
            ...
            $temp_data['profile_picture_path'] = $image;
            $this->db->where('id', $user_id);
            $this->db->update('users', $temp_data);
        }
    }
    ...
    public function subRoles()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if($chkStatus=="delete"){
                ...
                $where['id'] = $dataId;
                $chkRecord = $this->Userextramodel->deleteData("groups", $where);
                if ($chkRecord) {
                    $response['status'] = "success";
                ...
                $this->form_validation->set_rules('role_name', 'Role Name', 'required|is_unique[groups.name]');
                if ($this->form_validation->run() == FALSE) {
                    $response['status'] = "fail";
                    $response['message'] = strip_tags(validation_errors());
                } else {
                    $roleName = $this->input->post('role_name');
                    $insRecord['parent_id']= 92;
                    $insRecord['name'] = $roleName;
                    $insRecord['description'] = $roleName;
                    $insRecord['type_cate'] = 'category';
                    $insRecord['parent_group_type'] = 'HD';
                    if ($this->input->post('form_status') == "edit") {
                        $editId = $this->input->post('edit_id');
                        $chkRecord = $this->Userextramodel->updateTable('groups', $insRecord, array("id" => $editId));
                    } else {
                        $chkRecord = $this->Userextramodel->addRecord('groups', $insRecord);
                    }
                    if ($chkRecord) {
                        $response['status'] = "success";
...
//        $data['leaveGroups'] = $this->Userextramodel->leaveGroups();
//        $data['leaveTypes'] = $this->Userextramodel->leaveTypes();
        $data['allParentGroups'] = $this->db->where(array("parent_id"=>0,"id"=>92))->select("*")->get("groups")->result();
        $data['allGroups'] = $this->db->where(array("parent_id"=>92))->select("*")->get("groups")->result();

//        echo "<pre>";print_r($data);exit;

        $this->load->view('templates/header-new');
        $this->load->view('auth/sub_role_category', $data);
    ...
    public function inactive_user() {
        ...
        $email = $this->input->post('email');
        $this->db->delete('login_attempts', ['login'=>$email]);
        $res = $this->db->update('users', ['active' => 1], ['id' => $userId]);
        if($res){
            $response['status'] = "success";
    ...
    public function update_directory() {
        $userId = $this->input->post('id');
        $directory = $this->input->post('in_directory');
        $res = $this->db->update('users', ['in_directory' => $directory], ['id' => $userId]);
        if($res){
            $response['status'] = "success";
            $response['message'] = "In directory update successfully";
        }else{
            $response['status'] = "error";
            $response['message'] = "Something went wrong. Please try again.";
        }
