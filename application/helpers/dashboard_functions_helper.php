...
if (!function_exists('uralensis_get_Labrecord_status_counter')) {
     ...
     * @param string $type
     */
    function uralensis_get_Labrecord_status_counter($type = '', $is_return= '')
    {
        $ci = &get_instance();
        $ci->load->model('Laboratory_model');
        $user_id = $ci->ion_auth->user()->row()->id;
        if (!empty($type) && $type === 'reported') {
            $result = $ci->Laboratory_model->status_bar_result_count_published();
            if($is_return == 'return_me'){
                return $result;
            }else{
                echo $result;
            }            
        } elseif ($type === 'unreported') {
            $result = $ci->Laboratory_model->status_bar_result_count_unpublished();
            if($is_return == 'return_me'){
                return $result;
            }else{
                echo $result;
...
//        $count_data = $ci->Institute_model->further_view();
        ...
        $count_data = $ci->Doctor_model->display_further_work_model_requested_new();
...
function getTestAgsinstSubCatCustom($labId='')
{
...
function getTestAgsinstSubCatNew($labId, $mainCatId, $subCatId='')
{

    if(!empty($labId)){
        $CI = &get_instance();
        if(!empty($subCatId)){
            $query = "SELECT * FROM laboratory_tests WHERE lab_id = $labId AND category_id = $mainCatId AND sub_category_id = $subCatId";
        }else{
            $query = "SELECT * FROM laboratory_tests WHERE lab_id = $labId AND category_id = $mainCatId";
        }
        //$query = "SELECT * FROM laboratory_tests WHERE lab_id = $labId AND (specialty_id = $subCat OR sub_category_id = $subCat)";
...
function getSelectedLabsDetails($user_id)
{
    $CI = &get_instance();
    $query = "SELECT * FROM groups as gr join users_groups as us on gr.id=us.institute_id WHERE gr.group_type = 'L' and us.user_id='$user_id'";
...
if (!function_exists('getFlagCommentDetails')) {
    ...
    function getFlagCommentDetails($commentId,$postData,$dataSection=FALSE)
    {
        ...
        foreach ($time_data as $result){
            ...
            if($result->user_id==$log_user_id){
                $addMoreOpt = '<li class="list-inline-item">
                                                   <div class="like p-2 dropdown dropdown-action">
                                                        ...
                                                        <a class="dropdown-item edit-comment-btn hide" data-id="'.$result->id.'" data-recordid="'.$result->record_id.'" href="javascript:;"><i class="fa fa-pencil m-r-5"></i> Edit</a>
            ...
            $returnHtml .='<div class="bg-white p-2">
                                    <div class="user-info"><img class="rounded-circle pull-left img-rounded" style="margin-right:15px;" src="'.get_profile_picture($result->profile_picture_path, $result->first_name, $result->last_name).'" width="40">
                                        <div class="ml-2"><span class="font-weight-bold name">'.$result->first_name.' '.$result->last_name.'</span><span style="margin-left:10px;" class="date text-muted">'.date("d-M-Y",strtotime($result->created_date)).'</span>
                                            <ul class="list-inline pull-right timesheet_ul" style="font-size:15px">
                                                <li class="list-inline-item">
                                                    <div class="like p-2 cursor comment_like" data-id="'.$result->id.'" data-status="1" data-section="'.$dataSection.'" style="'.$likedColor.'">
                                                ...
                                                <li class="list-inline-item">
                                                    <div class="like p-2 cursor comment_like" data-id="'.$result->id.'" data-status="2" data-section="'.$dataSection.'" style="'.$disLikedColor.'">
                                    ...
                                    <div class="mt-2">
                                        <p class="comment-text">'.$result->description.'</p>
                                    </div>
                              </div>';
