...
if (!$this->uri->segment(1) == 'auth') { ?>
    <footer id="tg-footer" class="tg-footer tg-haslayout">
        ...
        <p class="tg-copyrights">Copyright © 2018 <span>Oxbridge LMS</span> All Rights Reserved.</p>
    </footer>
<?php } ?>
</div>
</body>
...
<?php if(isset($_SESSION['email_activation'])){ ?>
   <script>
       $.sticky('<?php echo $_SESSION["email_activation"]; ?>', {
           classList: 'success',
           speed: 200,
           autoclose: 7000
       });
   </script>
<?php unset($_SESSION['email_activation']); } ?>
...
<script>
    ...
    jQuery(document).ready(function () {
        ...
        jQuery('.collapse_item').click(function(e){
            var _this = $(this);
        ...
        jQuery('.hospitals_options').hide();
        jQuery(document).on('change', '.display_mdt_list_on_hospital', function(){
        ...
        jQuery(document).on('click', '.save_mdt_category', function(e){
            e.preventDefault();
            var _this = jQuery(this);
            var form_data = jQuery('.mdt_category_form').serialize();
            jQuery.ajax({
                type: "POST",
        ...
        jQuery(document).on('click', '.refresh_mdt_category_data', function(e){
            e.preventDefault();
            var _this = jQuery(this);
        ...
        jQuery(document).on('click', '.mdt_cat_delete', function(e){
            e.preventDefault();
            var _this = jQuery(this);
        ...
        $(document).on('click', '.create_user_btn', function (e) {
            ...
            if ($('input[name=first_name]').val() === '') {
                jQuery.sticky('First name must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=last_name]').val() === '') {
                jQuery.sticky('Last name must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=phone]').val() === '') {
                jQuery.sticky('Phone field must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=password]').val() === '') {
                jQuery.sticky('Password field must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=password_confirm]').val() === '') {
                jQuery.sticky('Password Confirm field must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=password]').val() !== $('input[name=password_confirm]').val()) {
                jQuery.sticky('Your Confirm Password Did Not Match', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=email]').val() === '') {
                jQuery.sticky('Email field must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
            if ($('input[name=memorable]').val() === '') {
                jQuery.sticky('Memorable field must not empty.', {classList: 'important', speed: 200, autoclose: 7000});
                return false;
            }
        ...
        function collapseMenu() {
            jQuery('.tg-navigation ul li.menu-item-has-children, .tg-navigation ul li.page_item_has_children, .tg-dashboardnav ul li.menu-item-has-children, .tg-dashboardnav ul li.page_item_has_children, .tg-navigation ul li.menu-item-has-mega-menu').prepend('<span class="tg-dropdowarrow"><i class="fa fa-angle-down"></i></span>');
            jQuery('.tg-navigation ul li.menu-item-has-children span, .tg-navigation ul li.page_item_has_children span, .tg-dashboardnav ul li.menu-item-has-children span, .tg-dashboardnav ul li.page_item_has_children span, .tg-navigation ul li.menu-item-has-mega-menu span').on('click', function () {
        ...
        $('ul.hospital_groups_cats li.menu-item-has-children').on('click', function () {
            jQuery(this).toggleClass('tg-open');
            jQuery(this).find('ul.hospital_groups_sub_menu').slideToggle(300);
        });
        /*--------------------------------------
         DASHBOARD MENU					
         --------------------------------------*/
        if (jQuery('#tg-btnmenutoggle').length > 0) {
            jQuery("#tg-btnmenutoggle").on('click', function (event) {
                ...
                jQuery('.tg-dashboardnav ul.sub-menu').hide();
            });
        }
        ...
        /* -------------------------------------
         OPEN CLOSE
         -------------------------------------- */
        jQuery('#tg-languagesbutton').on('click', function (event) {
            event.preventDefault();
            jQuery('.tg-langnotification li ul').slideToggle();
        });
        ...
        jQuery('#dob, #revalidation_date, #outsource_work_avail_date, #date_taken, #date_received_bylab, #data_processed_bylab, #date_sent_touralensis').datepick({
        ...
        jQuery('#admin_display_records').on('click', '.record_id_unpublish', function () {
        ...
        jQuery('#admin_display_records').on('click', '.record_id_delete', function () {
...
//                        _this.parents('.row').find('.courier_records_data #courier_records_table');
        ...
        ProfileUploader.bind('FilesAdded', function (up, files) {
            ...
            jQuery('.tg-useruploadimg').html(profileThumb);
            up.refresh();
        ...
        ProfileUploader.bind('FileUploaded', function (up, file, ajax_response) {
            ...
            if (response.success) {
                ...
                jQuery("#thumb-" + file.id).html(spinner_html);
                setTimeout(function(){ 
                    var img_html = '';
                    ...
                    jQuery('.tg-useruploadimg').find("div#thumb-" + file.id).removeClass('tg-thumboption');
                    jQuery("#thumb-" + file.id).html(img_html);
                    jQuery('.tg-userinfo-holder').find('.tg-user-img').html(user_img);
                    jQuery('input[name=profile_image_name]').val(response.file_name);
                    jQuery('input[name=profile_image_path]').val(response.full_path);
                 }, 1000);
            } else {
        ...
        //Delete Award Image
        jQuery(document).on('click', '.delete_profile_pic', function (e) {
            e.preventDefault();
            var _this = jQuery(this);
            _this.parents('.tg-useruploadimg').html('<i class="lnr lnr-user"></i>');
            var user_initial = jQuery('.tg-userinfo-holder').data('userinitial');
            jQuery('.tg-userinfo-holder').find('.tg-user-img').html('<h2>'+user_initial+'</h2>');
            jQuery('input[name=profile_image_name]').val('');
            jQuery('input[name=profile_image_path]').val('');
        });
...
<script>
    jQuery(document).ready(function () {
        ...
        $('input[name=tracking_no_lab]').bind('change', function (e) {
            ...
            var cct = $.cookie("<?php echo $this->config->item("csrf_cookie_name"); ?>");
            $.ajax({url: '<?php echo base_url('/index.php/admin/search_barcode_record'); ?>',
                type: 'POST',
                dataType: 'json',
        ...
        $(document).on('click', '.save_track_template', function (e) {
            ...
            if ($('#track_template_input').hasClass('in')) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'json',
                    data: {'hospital_user': hospital_user, 'clinic_users': clinic_users, 'pathologist': pathologist, 'lab_name': lab_name, 'report_urgency': report_urgency, 'specimen_type': specimen_type, 'input_name': input_name,'<?php echo $this->security->get_csrf_token_name(); ?>' : cct},
                    success: function (response) {
                        if (response.type === 'success') {
    ...
    $(document).ready(function () {
        ...
        $('input[name=lab_tracking_no_lab]').bind('change', function (e) {
            ...
            var search_type = 'lab_number';
            var cct = $.cookie("<?php echo $this->config->item("csrf_cookie_name"); ?>");
            $.ajax({url: '<?php echo base_url('/index.php/admin/lab_search_barcode_record'); ?>',
                type: 'POST',
                dataType: 'json',
                data: {'track_no_lab': track_no_lab, 'search_type': search_type,'<?php echo $this->security->get_csrf_token_name(); ?>' : cct},
                success: function (response) {
    ...
    $(document).ready(function () {
        ...
        $('input[name=doc_tracking_no_lab]').bind('change', function (e) {
            ...
            var search_type = 'lab_number';
            var cct = $.cookie("<?php echo $this->config->item("csrf_cookie_name"); ?>");
            $.ajax({url: '<?php echo base_url('/index.php/admin/doctor_search_barcode_record'); ?>',
                type: 'POST',
                dataType: 'json',
                data: {'track_no_lab': track_no_lab, 'search_type': search_type,'<?php echo $this->security->get_csrf_token_name(); ?>' : cct},
                success: function (response) {
    ...
    $(document).ready(function () {
        ...
        InvToSecUploader.bind('FilesAdded', function (up, files) {
            ...
            jQuery('.profile-to-sec-img-wrap').html(profileThumb);
            up.refresh();
        ...
        InvFromSecUploader.bind('FilesAdded', function (up, files) {
            ...
            jQuery('.profile-from-sec-img-wrap').html(profileThumb);
            up.refresh();
            ...
            onStepChanging: function (event, currentIndex, newIndex) {
                ...
                if (currentIndex === 0) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        async: false,
                        data: {'user_identity': identity, 'user_password': password, [csrf_name]: csrf_hash},
                        success: function (data) {
                            // console.log(data); return false;
                            if (data.type === 'success') {
                                jQuery.sticky(data.msg, {classList: 'success', speed: 200, autoclose: 5000});
                                if(data.login=="success"){
                                    jQuery.sticky("Successful login", {classList: 'success', speed: 200, autoclose: 5000});
                ...
                if (newIndex === 2) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        async: false,
                        data: {'user_identity': identity, 'user_password': password, 'memorable1': memorable1, 'memorable2': memorable2, 'hidden_mem1': hidden_memorable1, 'hidden_mem2': hidden_memorable2, [csrf_name]: csrf_hash},
                        success: function (data) {
            ...
            onFinishing: function (event, currentIndex) {
                ...
                if (currentIndex === 2) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        async: false,
                        data: {'verify_auth': verify_auth, 'user_identity': identity, 'user_password': password, 'remember_pc': remember_pc, [csrf_name]: csrf_hash},
                        success: function (data) {
...
<script type="text/javascript">
    $(document).ready(function(){
        $(".wizard > .actions a").text("Sign in");
        $(".cust_tooltip.login .fa").click(function(){
            $("#steps-uid-0-p-2").css("position","relative");
        });
    });
</script>
</html>