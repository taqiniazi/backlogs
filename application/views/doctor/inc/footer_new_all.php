    ...
    jQuery(document).ready(function () {
       ...
       function generateTxt(){
            ...
            $.ajax({
                type: "POST",
                ///*url: "<?//= base_url('patient/generate_hl7_file'); ?>//",*/
                url: "<?php echo base_url('/index.php/patient/generate_orm_file'); ?>",
        ...
        jQuery(document).on('change', '.microscopicCode', function(){
            ...
            $('#microscopicCodeId'+formId+' > option:selected').each(function() {
                description += ''+$(this).attr('data-tdesc');
                console.log()
            });
        ...
        jQuery(document).on('click', '.generate_obr', function () {
            let patientID = $(this).attr('data-pid');
            let requestID = $(this).attr('data-request-id');
            jQuery.ajax({
                type: "POST",
                url: '<?php echo base_url('patient/generate_obr_file'); ?>',
                data: { 'id': patientID, 'request_id': requestID, [csrf_name]: csrf_hash },
                dataType: "json",
                success: function (response){
                    if (response.status === 'success') {
                        jQuery.sticky(response.message, {classList: 'success', speed: 200, autoclose: 5000});
                        jQuery(document).find('#patient_obr').modal('hide');
                    } else {
                        jQuery.sticky(response.message, {classList: 'important', speed: 200, autoclose: 5000});
                    }
                }
        ...
        jQuery(document).on('click', '.read_obr', function () {
            let patientID = $(this).attr('data-pid');
            let requestID = $(this).attr('data-request-id');
            jQuery.ajax({
                type: "POST",
                url: '<?php echo base_url('patient/read_obr_file'); ?>',
                data: { 'id': patientID, 'request_id': requestID, [csrf_name]: csrf_hash },
                dataType: "json",
                success: function (response){
                    if (response.status == 'success') {
                        let modalDiv = jQuery(document).find('#patient_obr');
                        modalDiv.modal('show');
                        modalDiv.find('.pdata').text('');
                        modalDiv.find('#obr_title').text(response.title);
                        modalDiv.find('#generate_obr_div').hide();
                        modalDiv.find('#obr_content').show();
                        modalDiv.find('#obr_content').text(response.data);
                    } else if (response.status == 'warning'){
                        let modalDiv = jQuery(document).find('#patient_obr');
                        modalDiv.modal('show');
                        modalDiv.find('.pdata').text('');
                        modalDiv.find('#obr_title').text(response.title);
                        modalDiv.find('#obr_content').hide();
                        modalDiv.find('#generate_obr_div').show();
                        modalDiv.find('#generate_obr_div').html(response.data);
                    } else {
                        jQuery.sticky(response.message, {classList: 'important', speed: 200, autoclose: 5000});
                    }
                }
...
function add_request_clinician_table_data(data) {
    var row = $("<tr></tr>");
    var checked = '';
    if(data['clinician_id'] === data['dermatological_surgeon']) checked = 'checked';
    row.append('<td><input type="radio" name="clinician_radio" style="cursor:pointer" class="clinician_radio" value="'+data['clinician_id']+'" onchange="javascript:UpdatePrimaryClinician('+data['clinician_id']+')" '+checked+'/></td>');
    row.append('<td>'+data['uname']+'</td>');
    // row.append('<td>'+data['time']+'</td>');
    row.append('<td><img class="img-fluid avatar" src="'+data['u_id']+'"></td>');
    $("#request-clinician-table").find('tbody').append(row);
}
...
var block_status = null;
var clinician_status = null;
...
$('#add_clinician_modal').on("show.bs.modal", function(e) {
    update_request_clinician_table();
});

function UpdatePrimaryClinician(clinicianId){
    $.ajax({
        url: '<?php echo base_url('/index.php/doctor/updateClinician'); ?>',
        type: 'POST',
        dataType: 'json',
        data: {'clinicianId': clinicianId, requestId : $('#requestId').val(), [csrf_name]: csrf_hash},
        success: function (response) {
            if (response.status === 'success') {
                jQuery.sticky("Clinician has been updated!", {classList: 'success', speed: 200, autoclose: 7000});
            } else {
...
function update_request_clinician_table() {
    $("#request-clinician-table").find('tbody').html('');
    $.get(_base_url+`doctor/get_request_clinician?requestId=${encodeURIComponent($('#requestId').val())}`, function(data) {
        if (data.status == 'success') {
            ...
            if (data.clinicians.length == 0) {
                ...
                $("#request-clinician-table-heading").html('No Recorded History');
            }else{
                data.clinicians.forEach(function(clinicians) {
                    var user_id = clinicians['clinician_id'];
                    var u_id = '';
                    jQuery.ajaxSetup({async:false});
                    $.get(_base_url+'tracking/get_userid/'+user_id, function(data){
                        if (data.status == 'success') {
                        ...
                        add_request_clinician_table_data({
                            uname: clinicians['uname'],
                            dermatological_surgeon: clinicians['dermatological_surgeon'],
                            clinician_id : clinicians['clinician_id'],
                            u_id : u_id
                        });
                    });
                    jQuery.ajaxSetup({async:true});
...
$('.unpublish_table').DataTable({
   ...
   function gettatvalue(tatno)
   {
    ...
    // Start filter by weekly in published reports
      $('input[name="daterange"]').daterangepicker({
            opens: 'left'
        }, function(start, end, label) {
            load_ajax_publish_record_data('','','','',start.format('YYYY-MM-DD')+':'+end.format('YYYY-MM-DD'));
        //     $.ajax({
        //     type: "POST",
