        ...
        <table class="table table-striped no-footer" id="revision_table_datatable" style="width: 100%;">
            <thead>        
				...
				foreach($result as $row){
                    ...
                    <td><a href="<?php echo base_url('/Document_List?cat=').$row['id']; ?>"><?php echo count($row[0]);  ?></a></td>
                    <td><?= $row['icon']; ?></td>
                 	<td class="text-right">
					
					
					<a title="Add sub category" href="<?php echo base_url('Document_List/sub_category_section/'.$row['id'].'/0'); ?>" ><i class="fa fa-plus m-r-5"></i> </a>
					
					<a title="Edit category" href="<?php echo base_url('Document_List/category_section/'.$row['id']); ?>" ><i class="fa fa-edit m-r-5"></i> </a>
                    
					<a class="" href="javascript:delete_record('<?= base_url('Document_List/delete_category/'.$row['id']); ?>')" ><i class="fa fa-trash-o m-r-5"></i> </a></td>
				<?php 	
                  echo '</tr>';
				$sn++;
				 }	
