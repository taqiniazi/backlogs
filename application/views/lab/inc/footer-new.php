...
<script>
    ...
    // Click on payment_status checkbox
    $('body').on('change', '.payment_status', function() {
        ...
        if(this.checked) {
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "<?php echo base_url(); ?>/lab/change_payment_status",
                data: { status : 1,requestid: requestid, [csrf_name]:csrf_hash },
            });
        }
        else{
            $.ajax({
                ...
                dataType: "json",
                url: "<?php echo base_url(); ?>/lab/change_payment_status",
                data: { status : 0,requestid: requestid, [csrf_name]:csrf_hash},
            });
        }
        jQuery.sticky('Record Updated', {classList: 'success', speed: 200, autoclose: 2000});
    });
...
<script>
    ...
    $(document).ready(function () {
        ...
        function formatState (opt) {
            ...
            if(!optimage){
                ...
                var $opt = $('<span><img src="' + optimage + '" width="20px" /> ' + opt.text + '</span>');
                return $opt;
            }
        };
    ...
    function ajax_display_comment_box_hover() {
	 ...
	 function donwloadReport(requestID,patientID){
        $.ajax({
            type: "POST",
            ...
            success: function (response){
                if (response.status === 'success') {
                    jQuery.sticky("Report has been generated successfully!!!!", {classList: 'success', speed: 200, autoclose: 5000});
                } else {
                    jQuery.sticky("Something went wrong. Please try again!!!!", {classList: 'important', speed: 200, autoclose: 5000});
                }
            }
        });
    }
	
		 function donwloadReport2(requestID,patientID){
        $.ajax({
            type: "POST",
            ///*url: "<?//= base_url('patient/generate_hl7_file'); ?>//",*/
            url: "<?php echo base_url('/index.php/patient/generate_orm_file2'); ?>",
            data: { 'id': patientID, 'request_id': requestID, [csrf_name]:csrf_hash },
            dataType: "json",
            success: function (response){
                if (response.status === 'success') {
    ...
    function donwloadBilling(requestID,patientID){
        $.ajax({
            type: "POST",
            ///*url: "<?//= base_url('patient/generate_hl7_file'); ?>//",*/
            url: "<?php echo base_url('/index.php/patient/generate_billing_file'); ?>",
            data: { 'id': patientID, 'request_id': requestID, [csrf_name]:csrf_hash },
            dataType: "json",
            success: function (response){
                if (response.status === 'success') {
                    jQuery.sticky("Billing has been generated successfully!!!!", {classList: 'success', speed: 200, autoclose: 5000});
                } else {
                    jQuery.sticky("Billing went wrong. Please try again!!!!", {classList: 'important', speed: 200, autoclose: 5000});
                }
            }
        });
    }
    ...
    function ajax_add_flag_comments_box() {
        $(document).on('click', '.display_comment_box', function (e) {
            ...
            e.preventDefault();
            var _this = $(this);
            let dynamic_id = _this.data('modalid');
            $('#flag_comment_model-' + dynamic_id).modal('show');
            $('#flag_comment_model-' + dynamic_id).find('.flag_comment').text(_this.data('data-description'));
    ...
    function ajax_add_hl7_comments_box(){
        $(document).on('click', '.display_hl7_box', function (e) {
            //return false;
            e.preventDefault();
            var _this = $(this);
            let dynamic_id = _this.data('modalid');
            $('#flag_hl7_model-' + dynamic_id).modal('show');
            $('#flag_hl7_model-' + dynamic_id).find('.hl7_comment').text(_this.data('data-description'));
            $('#flag_hl7_model-' + dynamic_id).find('#hl7_csrf_name').val(csrf_hash);
            $(document).on('click', '.flag_hl7_save', function (e) {
                e.preventDefault();
                var _this = $(this);
                var form_data = $('#flag_hl7_model-' + dynamic_id).find('#flag_hl7_form_' + dynamic_id).serialize();
                

                if(1){
                    $.ajax({
                        url: '<?php echo base_url('/index.php/doctor/save_hl7_comments'); ?>',
                        type: 'POST',
                        global: false,
                        dataType: 'json',
                        data: form_data,
    ...
    function load_ajax_publish_record_data(flag_type, sort_authorize, urgency_type, row_color_code) {
        ...
        //         var rowCodeClass = aData[14];
        //         $('td', nRow).eq(14).addClass('flag_column');
        //         $('td', nRow).eq(11).addClass('flag_column');
        //         $('td', nRow).eq(12).addClass('hide_content');
        //         $('td', nRow).eq(13).addClass('flag_column hide_content');
        //         $('td', nRow).eq(14).addClass('hide_content');
        //         $(nRow).addClass(rowClass);
        //         $('td', nRow).eq(0).addClass(rowCodeClass);
        //         $(nRow).addClass(rowCodeClass);
        //     }
    ...
    function add_flag_comments_box() {
        $(document).on('click', '.display_comment_box', function (e) {
            ...
            let dynamic_id = _this.data('modalid');
            $('#flag_comment_model-' + dynamic_id).modal('show');
            $('#flag_comment_model-' + dynamic_id).find('.flag_comment').text(_this.data('description'));
    ...
    function filter_row_by_status_on_record_list(oTable) {
        $(document).on('click', '.filter_by_status_btn', function (e) {
            ...
            $.fn.dataTableExt.afnFiltering.push(function (oSettings, aData, iDataIndex) {
				...
				//alert(aData[2].trim());
				//alert(selected_hos_val);
                if (aData[15].trim() == selected_hos_val) {
                    return true;
                }
                return false;
            });
    ...
    $(document).ready(function(){
        ...
        function formatUsersList(user) {
            ...
            if(origin == 'http://localhost'){
                base_url = origin+'/fnqhpathology_latest/'; 
            }
            var full_picture_path = base_url + "/" + picture_path;
