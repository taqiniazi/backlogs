    ...
    <div class="row align-items-center">
        ...
        <div class="col-6 text-right">
            <div class="view-icons">
                <a href="javascript:void(0);" class="grid-view btn btn-link active" title="Grid View"><i class="fa fa-th"></i></a>
                <a href="javascript:void(0);" class="list-view btn btn-link" title="List View"><i class="fa fa-bars"></i></a>
            </div>
...
<div class="row">
    ...
    <div class="col-md-6 col-sm-6 col-lg-3 col-xl-3">
    <a href="<?php echo base_url('auth/dashoardDetails?group_type=CS'); ?>" target="_blank">
        <div class="card dash-widget">
            <div class="card-body">
                <span class="dash-widget-icon">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="text-right" style="width: 40px;">
                                <img data-toggle="tooltip" title="" src="<?php echo base_url();?>assets/icons/Actions-Blue.png" class="img-responsive" style="width: 40px;margin: 0 auto;" data-original-title="Actions">
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <h2 class="table-avatar">
                                    <div class="profile-img">
                                        <a href="javascript:void(0);" class="avatar">
                                            <img src="<?= get_profile_picture($value->profile_picture_path, $first_name, $last_name); ?>" alt="">
                                        </a>
                                    </div>
                                    <a href="javascript:void(0);">
                                        <?= $full_name; ?>
                                        <span><?= ucwords($value->description); ?></span>
                                        <span><?= (empty($value->active) ? 'Block' : '') ?></span>
                                    </a>
                                </h2>
                            </td>
                            <td>
                                <div class="hospital-container">
                                    <?php foreach ($related_hospital as $h) : ?>
                                        <div data-toggle="tooltip" data-placement="top" title="<?= $h['description']; ?>" class="hospital-info"><?= $h['first_initial'] . $h['last_initial'] ?></div>
                                    <?php endforeach; ?>
                                </div>
                            </td>
                            <td><?= $value->enc_email; ?> <br /> <?= $value->enc_phone; ?></td>
                            <!-- <td></td> -->
                            <!-- <td><?= date("d M Y", $value->created_on); ?></td> -->
                            <td>
                                <div class="dropdown">
                                    <button class="btn btn-white btn-sm btn-rounded dropdown-toggle" aria-expanded="false" disabled>
                                        <?= ucwords($value->description); ?>
                                    </button>
                                </div>
                            </td>
                            <td>
                                <?php if($value->wrong_attempt >= 3) { ?>
                                    <button type="button" class="btn btn-danger status-btn inactive-user" data-id="<?= $value->user_id; ?>" data-email="<?= $value->enc_email; ?>">Unblock</button>
                                <?php } else { ?>
                                    <button type="button" class="btn btn-success status-btn">Active</button>
                                <?php } ?>
                            </td>
                            <td class="text-center">
                                <input type="checkbox" class="update-directory" value="<?= $value->in_directory; ?>" data-id="<?= $value->user_id; ?>" <?= (!empty($value->in_directory) ? 'checked' :''); ?> />
                            </td>
                            <td class="text-right">
                                <div class="dropdown-menu-right">
                                    <a class="dropdown-item" href="<?= base_url('auth/edit_user'); ?>/<?= $value->user_id; ?>"><i class="fa fa-pencil m-r-5"></i></a>
                                    <a class="dropdown-item delete-user" href="javascript:void(0);" data-toggle="modal" data-id="<?= $value->user_id; ?>" data-target="#delete_employee"><i class="fa fa-trash-o m-r-5"></i> </a>
                                    <?php if($value->group_type !="M") { ?>
                                        <a class="dropdown-item" href="javascript:void(0);" onClick="loadasAdmin(<?= intval($value->user_id); ?>)"><i class="fa fa-user m-r-5"></i> </a>
                                    <?php }?>
                                </div>
                                <!-- <div class="dropdown dropdown-action">
                                    <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item" href="<?= base_url('auth/edit_user'); ?>/<?= $value->user_id; ?>"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                        <a class="dropdown-item delete-user" href="javascript:void(0);" data-toggle="modal" data-id="<?= $value->user_id; ?>" data-target="#delete_employee"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                        <?php if($value->group_type !="M") { ?>
                                            <a class="dropdown-item" href="javascript:void(0);" onClick="loadasAdmin(<?= intval($value->user_id); ?>)"><i class="fa fa-user m-r-5"></i> Login</a>
                                        <?php }?>
                                    </div>
                                </div> -->
                            </td>
...
<div id="grid_view" class="hidden show">
    ...
    <div class="row staff-grid-row">
        ...
        $last_name = htmlspecialchars($value->enc_last_name, ENT_QUOTES, 'UTF-8');
		$company_name = htmlspecialchars($value->enc_company, ENT_QUOTES, 'UTF-8');
        if (!empty($first_name) || !empty($last_name))
		{
            $full_name = $first_name . ' ' . $last_name;
        }
       ...
       //print $value->group_type;
        $hospital_linked_name = '';
        $related_hospital = get_related_hospital($value->user_id, $value->group_id, $value->group_type, $value->type_cate);
        //print_r($related_hospital); exit;
        //echo $this->db->last_query(); exit;
    	$valuedescription='';
        if ($value->group_type === 'H' || $value->group_type === 'HA')
    	{
            $valuedescription = 'Hospital System Admin';
            if ($value->is_hospital_admin)
    		{
                $value->description = 'Hospital System Admin';
            }
        }
        ...
        //		 $value->description = 'Lab Accounts';
    		  $valuedescription= 'Lab Accounts';
				...
				else if($value->group_type === 'L')
    	{
        //		 $value->description = 'Lab Accounts';
    		  $valuedescription= 'Lab System Admin';
    	}
		else if($value->group_type === 'HS')
    	{
        //		 $value->description = 'Lab Accounts';
    		  $valuedescription= 'Hospital Secretary';
    	}
        ...
        <!--                                    class="fa fa-key m-r-5"></i> Password</a>
    			    ...
    			    <div class="small text-muted"><!--Rol$company_namee-->
    				<?php
                        echo $company_name;
                        ?>
...
<div class="modal custom-modal fade" id="delete_employee" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <?php echo form_open(site_url('/'), array('id' => 'delete_user_form')); ?>
                <div class="form-header">
                    <h3>Delete User</h3>
                    <p>Are you sure want to delete?</p>
                </div>
                <div class="modal-btn delete-action">
                    <?php echo form_hidden('action', 'delete_user'); ?>
                    <input type="hidden" name="delete_user_id" id="delete_user_id" value=""/>
                    <div class="row">
                        <div class="col-6">
                            <a href="javascript:void(0);" class="btn btn-primary btn-user-delete continue-btn">Delete</a>
                        </div>
                        <div class="col-6">
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on('click', '.delete-user', function (){
            ...
            $('#delete_employee').find('#delete_user_id').val(userID);
        })
