...
function update_record_history_table(qr_code) {
    $.get(_base_url+`tracking/get_record_history?lab_number=${encodeURIComponent(qr_code)}`, function(data) {
        if (data.status == 'success') {
            var arr = ["lab-entry","specimen-labelling","cut-up-grossing","embedding","staining","quality-assurance","slide-scanner","reporting","admin-support","further-work"];
            arr.every(function(i) { $('.'+i).prop('disabled',false); return true; });
            ...
            if (data.history.length == 0) {
               ...
               $("#dv_add_remove_class").removeClass('col-lg-12');
                ...
                if (current_status ===  null && data.track_status.length > 0) {
                    ...
                    select_circle(btn);
                    // Disable all the prevoius divs
                    var arr = ["lab-entry","specimen-labelling","cut-up-grossing","embedding","staining","quality-assurance","slide-scanner","reporting","admin-support","further-work"];
                    arr.every(function(value) {
                        $('.'+value).attr('disabled','disabled');
                        if(value == track_status) return false;
                        else return true;
                    });
                }
